...
/* ── ACCORDION BUTTONS (substitutos do st.expander) ── */
...
/* ── EXPANDER icon: zeramos font-size de qualquer Material Icons que vazar ── */
...
# ═══════════════════════════════════════════════════════
    ...
    # ── MOBILE BOTTOM NAV ──
    _aba_at = st.session_state.get("func_aba","🏠  Início")
    st.markdown(f"""
    <div class="mob-nav">
        <button class="mob-nav-btn {'active' if '🏠' in _aba_at else ''}" onclick="window.location.href='?fn=inicio'">
            <span class="ico">🏠</span>Início</button>
        <button class="mob-nav-btn {'active' if '📋' in _aba_at else ''}" onclick="window.location.href='?fn=agenda'">
            <span class="ico">📋</span>Agenda</button>
        {'<button class="mob-nav-btn" onclick="window.location.href=\'?fn=empresa\'"><span class="ico">🏢</span>Empresa</button>' if tem_empresa else ''}
        <button class="mob-nav-btn {'active' if '💬' in _aba_at else ''}" onclick="window.location.href='?fn=chat'">
            <span class="ico">💬</span>Chat{'<span style="color:#ff2b2b;font-size:8px">●</span>' if nao_lidas_f > 0 else ''}</button>
        <button class="mob-nav-btn {'active' if '⚙️' in _aba_at else ''}" onclick="window.location.href='?fn=config'">
            <span class="ico">⚙️</span>Config</button>
    </div>
    """, unsafe_allow_html=True)
    ...
    _fn_map = {"inicio":"🏠  Início","agenda":"📋  Agenda","empresa":"🏢  Empresa",
               "config":"⚙️  Config"}
    if f"💬" in str(_fn_qp): _fn_map["chat"] = f"💬  Chat{'  ●' if nao_lidas_f > 0 else ''}"
    if _fn_qp == "chat":
        _target = f"💬  Chat{'  ●' if nao_lidas_f > 0 else ''}"
        if _target in abas_nav and st.session_state.get("func_aba") != _target:
            st.session_state["func_aba"] = _target
            st.query_params.clear(); st.rerun()
    elif _fn_qp in _fn_map:
        _t2 = _fn_map[_fn_qp]
        if _t2 in abas_nav and st.session_state.get("func_aba") != _t2:
            st.session_state["func_aba"] = _t2
            st.query_params.clear(); st.rerun()

    _cur_idx = abas_nav.index(st.session_state["func_aba"]) if st.session_state["func_aba"] in abas_nav else 0
    aba = st.sidebar.radio("NAVEGAÇÃO", abas_nav, index=_cur_idx, key="func_nav_r")
