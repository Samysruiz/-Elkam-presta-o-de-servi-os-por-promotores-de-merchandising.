...
# ---------------- MIGRAÇÃO DO BANCO ----------------
# Garante que a tabela agenda tem todas as colunas necessárias
...
if tipo == "admin":
    ...
    # ---------------- FUNCIONÁRIOS ----------------
        ...
        with st.expander("➕ Cadastrar novo funcionário"):
            ...
            st.caption("A senha padrão será **elkam123** — o funcionário pode alterar no próprio painel.")

            if st.button("Cadastrar funcionário"):
                if user_novo.strip() == "":
                    st.error("Preencha o nome de usuário")
                else:
                    user_lower = user_novo.strip().lower()
                    c.execute("SELECT usuario FROM usuarios WHERE usuario=?", (user_lower,))
                    if c.fetchone():
                        st.error(f"Já existe um usuário com o nome '{user_lower}'.")
                    else:
                        c.execute(
                            "INSERT INTO usuarios VALUES(?,?,?)",
                            (user_lower, "elkam123", "funcionario")
                        )
                        conn.commit()
                        st.success(f"Funcionário '{user_lower}' criado! Senha padrão: elkam123")
                        st.rerun()
