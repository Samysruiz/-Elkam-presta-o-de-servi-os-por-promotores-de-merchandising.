    ...
    # ── AGENDA ──
        ...
        for merc in dados_dia["mercado"].unique():
            ...
            if not merc_info.empty:
                end_merc  = merc_info.iloc[0]["endereco"]
                logo_merc = merc_info.iloc[0].get("logo","") or ""

            # Título do expander COM logo ao lado
            with st.expander(f"🏪  {merc}", expanded=True):
                ...
                with hc1:
                    if logo_merc and os.path.exists(logo_merc):
                        st.image(logo_merc, width=64)
                    else:
                        st.markdown("<div style='font-size:40px;text-align:center'>🏪</div>", unsafe_allow_html=True)
                with hc2:
                    if end_merc:
                        st.markdown(
                            f"<div style='color:#555;font-size:12px;padding-top:8px'>📍 {end_merc}</div>",
                            unsafe_allow_html=True)
                        st.markdown(f"[🗺️ Google Maps](https://www.google.com/maps/search/{end_merc.replace(' ','+')})")
                ...
                for i, r in prods.iterrows():
                    prod_nome = r["produto"]
                    # Foto de referência do produto (cadastrada pelo admin)
                    ...
                    with p_col1:
                        st.checkbox(prod_nome, key=f"ck_{dia_sel}_{i}")
                    with p_col2:
                        if not foto_ref.empty and os.path.exists(str(foto_ref.iloc[0]["foto"])):
                            st.image(str(foto_ref.iloc[0]["foto"]), width=90)
                ...
                # ── FOTO DA GÔNDOLA (pequena, expansível) ──
                with st.expander("📷 Foto da gôndola", expanded=False):
                    ...
                    if modo_foto == "📸 Câmera":
                        foto_cam = st.camera_input("Tirar foto", key=f"cam_{merc}_{dia_sel}",
                                                   label_visibility="collapsed")
                        if foto_cam:
                            with open(nome_foto,"wb") as f2: f2.write(foto_cam.getbuffer())
                            caminho = nome_foto
                    else:
                        foto_arq = st.file_uploader("Selecionar foto",
                                                    ...
                                                    label_visibility="collapsed")
                        if foto_arq:
                            with open(nome_foto,"wb") as f2: f2.write(foto_arq.getbuffer())
                            caminho = nome_foto
                            st.image(nome_foto, width=220)

                    # Apagar foto
                    if os.path.exists(nome_foto):
                        if st.button("🗑️ Apagar foto", key=f"del_foto_{merc}_{dia_sel}"):
                            os.remove(nome_foto)
                            st.success("Foto removida.")
                            st.rerun()
                        else:
                            caminho = nome_foto
                            st.image(nome_foto, width=220, caption="Foto atual")
