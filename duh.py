...
/* ── EXPANDER: elimina COMPLETAMENTE o ícone Material Icons que vaza como texto ── */

/* 1. Esconde o span do ícone toggle em todas as versões do Streamlit */
...
/* 2. O leak "keyboard_arrow_right" / "expand_more" vem de um <p> dentro do summary
   com font-family: Material Symbols — zeramos o font-size para esconder o glifo */
[data-testid="stExpander"] summary p,
[data-testid="stExpander"] summary span {
    font-family: 'Inter', sans-serif !important;
}
/* Esconde qualquer elemento com fonte Material Icons */
...
/* 3. Texto do label do expander — garantir visibilidade correta */
[data-testid="stExpander"] summary {
    padding: 14px 16px !important;
...
<!-- FIX: Remove Material Icons text that leaks into expander labels -->
<script>
(function fixExpanderIcons() {
    var ICON_WORDS = ["keyboard_arrow_right","keyboard_arrow_down",
                      "expand_more","expand_less","chevron_right","arrow_right"];
    function clean() {
        // Walk all text nodes and blank out icon glyph strings
        document.querySelectorAll('[data-testid="stExpander"] summary *').forEach(function(el) {
            if (el.childNodes) {
                el.childNodes.forEach(function(node) {
                    if (node.nodeType === 3) { // TEXT_NODE
                        var t = node.textContent.trim();
                        if (ICON_WORDS.indexOf(t) !== -1) node.textContent = '';
                    }
                });
            }
            // Also hide elements using Material font
            var cs = window.getComputedStyle(el);
            if (cs.fontFamily && cs.fontFamily.indexOf('Material') !== -1) {
                el.style.fontSize = '0';
                el.style.width = '0';
...
with st.sidebar:
    ...
    if st.button("Salvar senha", use_container_width=True):
        if nova.strip():
            db_exec("UPDATE usuarios SET senha=? WHERE usuario=?", (nova, usuario))
...
if tipo_efetivo == "admin":
    ...
    # Mobile nav para admin — query param based navigation
    _qp = st.query_params.get("adm_menu", "Dashboard")
    _menu_opts = ["Dashboard", "Funcionários", "Mercados", "Agenda", "Relatórios", "Fotos",
                  "Fotos de Produtos", "Chat", "Empresa", "Destinatários", "⚙️ Configurações"]
    if _qp not in _menu_opts:
        _qp = "Dashboard"

    # Sync sidebar with query param
    _menu_idx = _menu_opts.index(_qp)
    menu = st.sidebar.selectbox("MENU", _menu_opts, index=_menu_idx,
                                 key="adm_menu_sel")
    if menu != _qp:
        st.query_params["adm_menu"] = menu
        st.rerun()

    st.markdown(f"""
    <div class="mob-nav">
        <button class="mob-nav-btn {'active' if menu=='Dashboard' else ''}"
            onclick="window.location.href='?adm_menu=Dashboard'">
            <span class="ico">📊</span>Dash
        </button>
        <button class="mob-nav-btn {'active' if menu=='Agenda' else ''}"
            onclick="window.location.href='?adm_menu=Agenda'">
            <span class="ico">📋</span>Agenda
        </button>
        <button class="mob-nav-btn {'active' if menu=='Relatórios' else ''}"
            onclick="window.location.href='?adm_menu=Relatórios'">
            <span class="ico">📈</span>Relat.
        </button>
        <button class="mob-nav-btn {'active' if menu=='Chat' else ''}"
            onclick="window.location.href='?adm_menu=Chat'">
            <span class="ico">💬</span>Chat
        </button>
        <button class="mob-nav-btn {'active' if menu=='⚙️ Configurações' else ''}"
            onclick="window.location.href='?adm_menu=⚙️ Configurações'">
    ...
    if menu == "Dashboard":
        ...
        _hoje_d = date.today()
        _ontem_d = _hoje_d - timedelta(days=1)
        _ini_sem_d = _hoje_d - timedelta(days=_hoje_d.weekday())
        _ini_ant_d = _ini_sem_d - timedelta(days=7)
        _fim_ant_d = _ini_sem_d - timedelta(days=1)

        _rel_all = db_read("SELECT * FROM relatorio")
        _ci_all  = db_read("SELECT * FROM checkin")
        if not _rel_all.empty:
            _rel_all["_d"] = pd.to_datetime(_rel_all["data"]).dt.date
        if not _ci_all.empty:
            _ci_all["_d"] = pd.to_datetime(_ci_all["data"]).dt.date

        def _fday(df, d):
            return df[df["_d"]==d] if not df.empty and "_d" in df.columns else pd.DataFrame()
        def _frange(df, d1, d2):
            return df[(df["_d"]>=d1)&(df["_d"]<=d2)] if not df.empty and "_d" in df.columns else pd.DataFrame()

        _rh = _fday(_rel_all, _hoje_d)
        _ro = _fday(_rel_all, _ontem_d)
        _rs = _frange(_rel_all, _ini_sem_d, _hoje_d)
        _ra = _frange(_rel_all, _ini_ant_d, _fim_ant_d)

        # ── MÉTRICAS DO DIA ──
        section_title("📊 Hoje")
        _d1,_d2,_d3,_d4,_d5 = st.columns(5)
        _d1.metric("Relatórios hoje", len(_rh), delta=f"{len(_rh)-len(_ro):+d} vs ontem")
        _fa_h = _rh["funcionario"].nunique() if not _rh.empty else 0
        _fa_o = _ro["funcionario"].nunique() if not _ro.empty else 0
        _d2.metric("Funcionários ativos", _fa_h, delta=f"{_fa_h-_fa_o:+d} vs ontem")
        _ma_h = _rh["mercado"].nunique() if not _rh.empty else 0
        _ma_o = _ro["mercado"].nunique() if not _ro.empty else 0
        _d3.metric("Mercados visitados", _ma_h, delta=f"{_ma_h-_ma_o:+d} vs ontem")
        _pf_h = len(_rh[_rh["status"]=="falta"]) if not _rh.empty else 0
        _pf_o = len(_ro[_ro["status"]=="falta"]) if not _ro.empty else 0
        _d4.metric("Produtos em falta", _pf_h, delta=f"{_pf_h-_pf_o:+d} vs ontem", delta_color="inverse")
        _ft_h = len(_rh[_rh["foto"]!=""]) if not _rh.empty else 0
        _ft_o = len(_ro[_ro["foto"]!=""]) if not _ro.empty else 0
        _d5.metric("Fotos enviadas", _ft_h, delta=f"{_ft_h-_ft_o:+d} vs ontem")

        # Último relatório
        _ci_h = _fday(_ci_all, _hoje_d)
        if not _ci_h.empty:
            _ult_ci = _ci_h.sort_values("id", ascending=False).iloc[0]
            st.markdown(f"""
            <div style='background:#111;border:1px solid #1c1c1c;border-radius:12px;
                        padding:14px 18px;margin:16px 0;display:flex;gap:16px;align-items:center'>
                <div style='font-size:28px'>⏱️</div>
                <div>
                    <div style='color:#444;font-size:10px;text-transform:uppercase;
                                letter-spacing:1px;margin-bottom:2px'>Último relatório registrado</div>
                    <div style='color:#fff;font-size:15px;font-weight:700'>{_ult_ci.get("mercado","—")}</div>
                    <div style='color:#555;font-size:12px'>
                        👤 {_ult_ci.get("funcionario","—")} · {_ult_ci.get("hora_entrada","—")}
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        if not _rh.empty:
            ...
            for _ri, (_, _rrow) in enumerate(_vf.iterrows()):
                ...
                _md = ["🥇","🥈","🥉"][_ri] if _ri < 3 else "👤"
                st.markdown(f"""
                <div style='margin-bottom:10px'>
                    <div style='display:flex;justify-content:space-between;margin-bottom:4px'>
                        <span style='color:#ddd;font-size:13px;font-weight:600'>{_md} {_rrow["funcionario"].split(".")[0].capitalize()}</span>
                        <span style='color:#ff2b2b;font-size:13px;font-weight:700'>{int(_rrow["n"])} visitas</span>
                    </div>
                    <div style='background:#1a1a1a;border-radius:6px;height:6px'>
                        <div style='background:#ff2b2b;width:{_pct}%;height:6px;border-radius:6px'></div>
                    </div>
                </div>""", unsafe_allow_html=True)
        else:
            st.info("Nenhuma visita registrada hoje.")

        # Em visita agora
        _ci_ativos = db_read("SELECT * FROM checkin WHERE data=? AND status='em_visita'", (_hoje_d,))
        if not _ci_ativos.empty:
            st.markdown("<hr style=\'border-color:#1c1c1c;margin:24px 0\'>", unsafe_allow_html=True)
            section_title("📍 Em visita agora")
            for _, _cir in _ci_ativos.iterrows():
                st.markdown(f"""
                <div style='background:#0a1f0a;border:1px solid #22c55e33;border-radius:10px;
                            padding:12px 16px;margin-bottom:8px;display:flex;align-items:center;gap:12px'>
                    <div style='width:8px;height:8px;background:#22c55e;border-radius:50%'></div>
                    <div>
                        <div style='color:#ddd;font-size:13px;font-weight:600'>👤 {_cir["funcionario"].split(".")[0].capitalize()}</div>
                        <div style='color:#555;font-size:12px'>🏪 {_cir["mercado"]} · desde {_cir["hora_entrada"]}</div>
                    </div>
                </div>""", unsafe_allow_html=True)
        ...
        _dias_lbl = ["Seg","Ter","Qua","Qui","Sex","Sáb","Dom"]
        def _pdias(df):
            if df.empty or "_d" not in df.columns: return [0]*7
            return [len(df[df["_d"]==(_ini_sem_d+timedelta(days=_di))]) for _di in range(7)]
        _sa = _pdias(_rs); _sb = _pdias(_ra)
        st.bar_chart(pd.DataFrame({"Semana atual":_sa,"Anterior":_sb}, index=_dias_lbl))
        _ta = sum(_sa); _tb = sum(_sb)
        _sc1,_sc2,_sc3 = st.columns(3)
        _sc1.metric("Semana atual", _ta)
        _sc2.metric("Semana anterior", _tb)
        _sc3.metric("Variação", f"{_ta-_tb:+d}")

        # Mensagens não lidas
        _nao_lidas = db_read("SELECT COUNT(*) as t FROM chat WHERE tipo='funcionario' AND lido=0").iloc[0]["t"]
        if _nao_lidas > 0:
            st.warning(f"💬 **{int(_nao_lidas)}** mensagem(ns) não lida(s) no Chat.")
        ...
        with st.expander("➕ Novo mercado"):
            ...
            if st.button("Cadastrar mercado"):
                if not mercado.strip() or not endereco.strip():
                    ...
                    dup = conn.cursor().execute("SELECT mercado FROM mercados WHERE endereco=?", (endereco.strip(),)).fetchone()
                    if dup:
                        st.error(f"⚠️ Endereço já cadastrado para **'{dup[0]}'**. O nome pode repetir, mas o endereço deve ser único.")
                    else:
                        try:
                            logo_path = ""
                            if logo_up:
...
# ═══════════════════════════════════════════════════════
    ...
    # ── MOBILE BOTTOM NAV ──
    _aba_ativa = st.session_state.get("func_aba","🏠  Início")
    st.markdown(f"""
    <div class="mob-nav">
        <button class="mob-nav-btn {'active' if '🏠' in _aba_ativa else ''}"
            onclick="window.location.href='?fn=inicio'">
            <span class="ico">🏠</span>Início
        </button>
        <button class="mob-nav-btn {'active' if '📋' in _aba_ativa else ''}"
            onclick="window.location.href='?fn=agenda'">
            <span class="ico">📋</span>Agenda
        </button>
        {'<button class="mob-nav-btn" onclick="window.location.href=\'?fn=empresa\'"><span class="ico">🏢</span>Empresa</button>' if tem_empresa else ''}
        <button class="mob-nav-btn {'active' if '💬' in _aba_ativa else ''}"
            onclick="window.location.href='?fn=chat'">
            <span class="ico">💬</span>Chat
            {'<span style="color:#ff2b2b;font-size:8px">●</span>' if nao_lidas_f > 0 else ''}
        </button>
        <button class="mob-nav-btn {'active' if '⚙️' in _aba_ativa else ''}"
            onclick="window.location.href='?fn=config'">
            <span class="ico">⚙️</span>Config
        </button>
    </div>
    """, unsafe_allow_html=True)
    ...
    _fn_qp = st.query_params.get("fn", "")
    _fn_map = {"inicio":"🏠  Início","agenda":"📋  Agenda","empresa":"🏢  Empresa",
               "chat":f"💬  Chat{'  ●' if nao_lidas_f > 0 else ''}","config":"⚙️  Config"}
    if _fn_qp in _fn_map:
        _target = _fn_map[_fn_qp]
        if _target in abas_nav and st.session_state.get("func_aba") != _target:
            st.session_state["func_aba"] = _target
            st.query_params.clear()
            st.rerun()

    abas_nav = ["🏠  Início"]
    abas_nav.append("📋  Agenda")
    if tem_empresa:
        abas_nav.append("🏢  Empresa")
    abas_nav.append(f"💬  Chat{'  ●' if nao_lidas_f > 0 else ''}")
    abas_nav.append("⚙️  Config")

    # Sair button in sidebar
    with st.sidebar:
        if st.button("🚪 Sair", use_container_width=True, key="func_sair"):
            st.session_state["logado"] = False
            st.rerun()
        st.markdown("<hr style='border-color:#1c1c1c'>", unsafe_allow_html=True)

    # Back navigation: when inside a mercado, show back button
    if "func_aba" not in st.session_state:
        st.session_state["func_aba"] = "🏠  Início"

    aba = st.sidebar.radio("NAVEGAÇÃO", abas_nav,
                           index=abas_nav.index(st.session_state["func_aba"])
                           if st.session_state["func_aba"] in abas_nav else 0,
                           key="func_nav_radio")
    st.session_state["func_aba"] = aba
    ...
    if aba == "🏠  Início":
        ...
        _dia_semana = ["Segunda","Terça","Quarta","Quinta","Sexta","Sábado","Domingo"][date.today().weekday()]

        # Métricas do dia
        _tarefas_hoje = db_read("SELECT * FROM agenda WHERE funcionario=?", (usuario,))
        _merc_dia = _tarefas_hoje[_tarefas_hoje["dia"]==_dia_semana]["mercado"].unique().tolist() if not _tarefas_hoje.empty else []
        _total_merc = len(_merc_dia)

        _rel_hoje = db_read("SELECT * FROM relatorio WHERE funcionario=? AND data=?", (usuario, _hoje))
        _visitados = _rel_hoje["mercado"].nunique() if not _rel_hoje.empty else 0
        _fotos_hoje = db_read("SELECT * FROM relatorio WHERE funcionario=? AND data=? AND foto!=''", (usuario, _hoje))
        _n_fotos = len(_fotos_hoje)
        _falta_hoje = db_read("SELECT * FROM relatorio WHERE funcionario=? AND data=? AND status='falta'", (usuario, _hoje))
        _n_falta = len(_falta_hoje)

        # Cards de métricas
        _m1, _m2, _m3, _m4 = st.columns(4)
        _m1.metric("Visitas hoje", f"{_visitados}/{_total_merc}")
        _m2.metric("Fotos enviadas", _n_fotos)
        _m3.metric("Produtos em falta", _n_falta)
        _checkins_h = db_read("SELECT * FROM checkin WHERE funcionario=? AND data=?", (usuario, _hoje))
        _m4.metric("Check-ins", len(_checkins_h))

        # Barra de progresso
        if _total_merc > 0:
            _prog = _visitados / _total_merc
            st.markdown(f"""
            <div style='margin:16px 0 8px'>
                <div style='display:flex;justify-content:space-between;margin-bottom:6px'>
                    <span style='color:#888;font-size:12px'>Progresso da rota</span>
                    <span style='color:#ff2b2b;font-size:12px;font-weight:700'>{int(_prog*100)}%</span>
                </div>
                <div style='background:#1a1a1a;border-radius:8px;height:8px;overflow:hidden'>
                    <div style='background:linear-gradient(90deg,#ff2b2b,#ff6b6b);
                                width:{int(_prog*100)}%;height:100%;border-radius:8px;
        ...
        # Próxima visita
        _merc_pendentes = [m for m in _merc_dia if m not in (_rel_hoje["mercado"].tolist() if not _rel_hoje.empty else [])]
        if _merc_pendentes:
            _prox = _merc_pendentes[0]
            _info_prox = db_read("SELECT endereco FROM mercados WHERE mercado=?", (_prox,))
            _end_prox = _info_prox.iloc[0]["endereco"] if not _info_prox.empty else ""
            _maps_p = f"https://www.google.com/maps/search/{_end_prox.replace(' ','+')}" if _end_prox else "#"
            st.markdown(f"""
            <div style='background:linear-gradient(135deg,#140000,#1e0000);
                        border:1.5px solid #ff2b2b44;border-radius:16px;
                        padding:20px 22px;margin-bottom:16px'>
                <div style='color:#ff2b2b;font-size:10px;font-weight:700;
                            letter-spacing:2px;text-transform:uppercase;margin-bottom:8px'>
                    📍 Próxima visita
                </div>
                <div style='color:#fff;font-size:20px;font-weight:900;margin-bottom:6px'>{_prox}</div>
                <div style='color:#444;font-size:12px;margin-bottom:16px'>{_end_prox}</div>
            </div>
            """, unsafe_allow_html=True)
            _pb1, _pb2 = st.columns(2)
            with _pb1:
                if _end_prox:
                    st.markdown(f"<a href='{_maps_p}' target='_blank'><button style='width:100%;background:#1a1a1a;color:#3b82f6;border:1px solid #1e3a5f;border-radius:10px;padding:12px;font-size:13px;font-weight:600;cursor:pointer'>🗺️ Ver rota</button></a>", unsafe_allow_html=True)
            with _pb2:
                if st.button("📷 Registrar visita", key="dash_registrar", use_container_width=True):
                    st.session_state["func_aba"] = "📋  Agenda"
                    st.session_state["dia_sel_dash"] = _dia_semana
                    st.session_state["merc_sel_dash"] = _prox
                    st.rerun()
        else:
            if _total_merc > 0:
                st.markdown("""
                <div style='background:#0a2b14;border:1px solid #22c55e33;border-radius:14px;
                            padding:20px;text-align:center'>
                    <div style='font-size:36px;margin-bottom:8px'>🏆</div>
                    <div style='color:#22c55e;font-size:16px;font-weight:800'>Rota concluída!</div>
                    <div style='color:#555;font-size:13px;margin-top:6px'>Todos os mercados foram visitados hoje.</div>
                </div>
                """, unsafe_allow_html=True)
        ...
        if _merc_dia:
            st.markdown("<div style='color:#444;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:12px'>📅 Agenda de hoje</div>", unsafe_allow_html=True)
            for _m in _merc_dia:
                _visitado = _m in (_rel_hoje["mercado"].tolist() if not _rel_hoje.empty else [])
                _cor = "#22c55e" if _visitado else "#444"
                _icone = "✅" if _visitado else "⏳"
                st.markdown(f"""
                <div style='background:#0d0d0d;border:1px solid #1a1a1a;border-radius:10px;
                    ...
                    <span style='font-size:18px'>{_icone}</span>
                    <span style='color:{"#ddd" if not _visitado else "#555"};
                                 font-size:14px;font-weight:600;
                                 {"text-decoration:line-through" if _visitado else ""}'>{_m}</span>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.info(f"Nenhuma agenda para {_dia_semana}.")

        # Última visita
        if not _rel_hoje.empty:
            _ult = db_read("SELECT * FROM checkin WHERE funcionario=? AND data=? ORDER BY id DESC LIMIT 1", (usuario, _hoje))
            if not _ult.empty:
                _ur = _ult.iloc[0]
                st.markdown(f"""
                <div style='margin-top:16px;background:#111;border:1px solid #1c1c1c;
                            border-radius:10px;padding:14px 16px'>
                    <div style='color:#444;font-size:10px;text-transform:uppercase;
                                letter-spacing:1px;margin-bottom:4px'>Última atividade</div>
                    <div style='color:#ddd;font-size:13px;font-weight:600'>
                        🏪 {_ur.get('mercado','—')}
                    </div>
                    <div style='color:#555;font-size:11px;margin-top:2px'>
                        Entrada {_ur.get('hora_entrada','—')}
                        {' · Saída '+str(_ur.get('hora_saida','')) if _ur.get('hora_saida') else ''}
                    </div>
                </div>
                """, unsafe_allow_html=True)

        # Ranking do dia
        _rank_hoje = db_read("SELECT funcionario, COUNT(*) as n FROM relatorio WHERE data=? GROUP BY funcionario ORDER BY n DESC", (_hoje,))
        if not _rank_hoje.empty and len(_rank_hoje) > 1:
            st.markdown("<hr style='border-color:#1c1c1c;margin:20px 0'>", unsafe_allow_html=True)
            st.markdown("<div style='color:#444;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:10px'>🏅 Ranking do dia</div>", unsafe_allow_html=True)
            for _ri, (_rr_i, _rr) in enumerate(_rank_hoje.iterrows()):
                _is_me = _rr["funcionario"] == usuario
                _medal = ["🥇","🥈","🥉"][_ri] if _ri < 3 else f"{_ri+1}º"
                st.markdown(f"""
                <div style='background:{"#1a0800" if _is_me else "#0d0d0d"};
                            border:1px solid {"#ff2b2b44" if _is_me else "#1a1a1a"};
                            border-radius:8px;padding:10px 14px;margin-bottom:6px;
                            display:flex;align-items:center;justify-content:space-between'>
                    <span style='color:{"#ff2b2b" if _is_me else "#888"};font-size:13px;font-weight:600'>
                        {_medal} {_rr["funcionario"].split(".")[0].capitalize()}
                        {"(você)" if _is_me else ""}
                    </span>
                    <span style='color:#555;font-size:12px'>{int(_rr["n"])} visitas</span>
                </div>
                """, unsafe_allow_html=True)
    ...
    # ── CONFIG FUNCIONÁRIO ──
        ...
        with _fcs1:
            _fc_nova = st.text_input("Nova senha", type="password", key="func_cfg_nova",
                                      placeholder="Nova senha...")
        with _fcs2:
            _fc_conf = st.text_input("Confirmar", type="password", key="func_cfg_conf",
                                      placeholder="Confirme...")
        if st.button("💾 Salvar senha", use_container_width=True, key="func_cfg_save"):
            if not _fc_nova or not _fc_conf:
                st.error("Preencha os dois campos.")
            elif _fc_nova != _fc_conf:
                st.error("Senhas não coincidem.")
            elif len(_fc_nova) < 4:
                st.error("Mínimo 4 caracteres.")
            else:
