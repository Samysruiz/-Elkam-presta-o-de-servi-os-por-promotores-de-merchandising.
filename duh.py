...
if "produto_faltante" not in relatorio_cols:
    ...
    # committed by db_exec
if "foto_preco" not in relatorio_cols:
    db_exec("ALTER TABLE relatorio ADD COLUMN foto_preco TEXT DEFAULT ''")
if "preco_valor" not in relatorio_cols:
    db_exec("ALTER TABLE relatorio ADD COLUMN preco_valor TEXT DEFAULT ''")

# ── Migrações para clientes ──
try:
    _cli_cols = [r[1] for r in conn.cursor().execute("PRAGMA table_info(clientes)").fetchall()]
    if "telefone" not in _cli_cols:
        db_exec("ALTER TABLE clientes ADD COLUMN telefone TEXT DEFAULT ''")
    if "primeiro_acesso" not in _cli_cols:
        db_exec("ALTER TABLE clientes ADD COLUMN primeiro_acesso INTEGER DEFAULT 0")
except: pass
...
if tipo == "cliente":
    _cli_pa_row = conn.cursor().execute(
        "SELECT primeiro_acesso FROM clientes WHERE LOWER(usuario)=LOWER(?)", (usuario,)).fetchone()
...
if tipo_efetivo == "admin":
    ...
    # ── RELATÓRIOS ────────────────────────────────────────
        ...
        rel_all = db_read(
            "SELECT rowid, * FROM relatorio WHERE funcionario NOT IN "
        ...
        if is_superadmin:
            _rel_teste = db_read(
                "SELECT rowid,* FROM relatorio WHERE funcionario IN "
                "(SELECT usuario FROM usuarios WHERE tipo='teste')")
