...
def db_exec(sql: str, params=(), retries: int = 5):
    sql = _fix_sql(sql)
...
def db_read(sql: str, params=()) -> pd.DataFrame:
    ...
    _conn = _pool.getconn()
    try:
        return pd.read_sql(sql, _conn, params=list(params) if params else None)
    except Exception:
        return pd.DataFrame()
    finally:
        _pool.putconn(_conn)

def db_one(sql: str, params=()) -> tuple | None:
...
class _FakeCursor:
    def execute(self, sql, params=()): db_exec(sql, params)
    def fetchone(self): return None

c = _FakeCursor()

db_exec("""CREATE TABLE IF NOT EXISTS usuarios (
    usuario TEXT, senha TEXT, tipo TEXT, primeiro_acesso INTEGER DEFAULT 1,
    aniversario TEXT DEFAULT '')""")
db_exec("""CREATE TABLE IF NOT EXISTS mercados (
    id SERIAL PRIMARY KEY, mercado TEXT, endereco TEXT UNIQUE, logo TEXT DEFAULT '',
    telefone_rel TEXT DEFAULT '', email_rel TEXT DEFAULT '')""")
db_exec("""CREATE TABLE IF NOT EXISTS produtos (mercado TEXT, produto TEXT)""")
db_exec("""CREATE TABLE IF NOT EXISTS produto_fotos (
    produto TEXT, foto TEXT)""")
db_exec("""CREATE TABLE IF NOT EXISTS agenda (
    id SERIAL PRIMARY KEY, funcionario TEXT, dia TEXT, mercado TEXT, produto TEXT)""")
db_exec("""CREATE TABLE IF NOT EXISTS relatorio (
    id SERIAL PRIMARY KEY, data TEXT, funcionario TEXT, mercado TEXT, produto TEXT, status TEXT, foto TEXT,
    ...
    foto_preco TEXT DEFAULT '', preco_valor TEXT DEFAULT '')""")
db_exec("""CREATE TABLE IF NOT EXISTS auditoria (
    id SERIAL PRIMARY KEY,
    data TEXT, hora TEXT, funcionario TEXT, mercado TEXT, tipo TEXT, detalhe TEXT)""")
db_exec("""CREATE TABLE IF NOT EXISTS chat (
    id SERIAL PRIMARY KEY,
    data TEXT, hora TEXT, remetente TEXT, tipo TEXT, mensagem TEXT, lido INTEGER DEFAULT 0)""")
db_exec("""CREATE TABLE IF NOT EXISTS checkin (
    id SERIAL PRIMARY KEY,
    data TEXT, hora_entrada TEXT, hora_saida TEXT,
    funcionario TEXT, mercado TEXT, status TEXT)""")
db_exec("""CREATE TABLE IF NOT EXISTS destinatarios (
    id SERIAL PRIMARY KEY, nome TEXT, telefone TEXT)""")
db_exec("""CREATE TABLE IF NOT EXISTS clientes (
    id SERIAL PRIMARY KEY,
    nome TEXT, email TEXT DEFAULT '',
    usuario TEXT UNIQUE, senha TEXT,
    mercados TEXT DEFAULT '',
    ativo INTEGER DEFAULT 1,
    telefone TEXT DEFAULT '',
    primeiro_acesso INTEGER DEFAULT 1)""")
db_exec("""CREATE TABLE IF NOT EXISTS empresa (
    id SERIAL PRIMARY KEY, nome TEXT, descricao TEXT, foto TEXT, concluida INTEGER DEFAULT 0)""")
db_exec("""CREATE TABLE IF NOT EXISTS config (
    chave TEXT PRIMARY KEY, valor TEXT)""")
db_exec("""CREATE TABLE IF NOT EXISTS agenda_template (
    id SERIAL PRIMARY KEY,
    nome_empresa TEXT, funcionario TEXT, dia TEXT, mercado TEXT, produto TEXT)""")
# committed by db_exec

# ── MIGRAÇÕES — PostgreSQL usa information_schema ──
def _col_exists(tabela: str, coluna: str) -> bool:
    r = db_read(
...
for _mig in [
    ...
    try:
        if not _col_exists(_mig[0], _mig[1]):
            db_exec(f"ALTER TABLE {_mig[0]} ADD COLUMN {_mig[1]} {_mig[2]}")
    except: pass
...
def obter_config(chave: str, padrao: str = "") -> str:
    r = db_one("SELECT valor FROM config WHERE chave=?", (chave,))
    return r[0] if r else padrao

def salvar_config(chave: str, valor: str):
    db_exec("INSERT INTO config (chave,valor) VALUES(?,?) ON CONFLICT (chave) DO UPDATE SET valor=EXCLUDED.valor", (chave, valor))
    # committed by db_exec
        ...
        if st.session_state["tela_login"] == "login":
            ...
            if st.button("ENTRAR →", use_container_width=True):
                ...
                _user_df = db_read(
                    ...
                    (u, senha_input))
                if not _user_df.empty:
                    user = _user_df.iloc[0]
                    ...
                    _cli_df = db_read(
                        "SELECT usuario FROM clientes WHERE LOWER(usuario)=LOWER(?) AND senha=? AND ativo=1",
                        (u, senha_input))
                    if not _cli_df.empty:
                        st.session_state["logado"]  = True
                        st.session_state["usuario"] = _cli_df.iloc[0]["usuario"]
...
# ── Primeiro acesso para CLIENTES ──
if tipo == "cliente":
    _cli_pa_row = db_one(
        "SELECT primeiro_acesso FROM clientes WHERE LOWER(usuario)=LOWER(?)", (usuario,))
    if _cli_pa_row and _cli_pa_row[0] == 1:
        st.markdown("<div style='height:60px'></div>", unsafe_allow_html=True)
        col_pa2, _ = st.columns([1, 2])
        with col_pa2:
            st.markdown("""
            <div style='margin-bottom:28px'>
                ...
                <div style='color:#444;font-size:13px'>Sua senha padrão é <b style="color:#ff2b2b">elkam</b>. Por segurança, defina uma senha pessoal para continuar.</div>
            </div>
            """, unsafe_allow_html=True)
            nova_s_cli = st.text_input("Nova senha", type="password", key="cli_pa_nova")
            conf_s_cli = st.text_input("Confirme a senha", type="password", key="cli_pa_conf")
            if st.button("Salvar e entrar →", use_container_width=True, key="cli_pa_btn"):
                if not nova_s_cli or not conf_s_cli:
                    st.error("Preencha os dois campos.")
                elif nova_s_cli != conf_s_cli:
                    st.error("As senhas não coincidem.")
                elif len(nova_s_cli) < 4:
                    st.error("Mínimo de 4 caracteres.")
                else:
                    db_exec("UPDATE clientes SET senha=?, primeiro_acesso=0 WHERE LOWER(usuario)=LOWER(?)",
                            (nova_s_cli, usuario))
                    st.success("Senha definida! Entrando...")
                    st.rerun()
        st.stop()

_pa_df = db_read("SELECT primeiro_acesso FROM usuarios WHERE usuario=?", (usuario,))
row_pa = tuple(_pa_df.iloc[0]) if not _pa_df.empty else None
...
with st.sidebar:
    ...
    """, unsafe_allow_html=True)
    # ── Botão Sair — sempre visível, topo do sidebar ──
    if st.button("🚪 Sair", key="sair_global", use_container_width=True):
        for _k in list(st.session_state.keys()):
            del st.session_state[_k]
        st.rerun()
    st.markdown("<div style='border-top:1px solid #1c1c1c;margin:10px 0'></div>",
                unsafe_allow_html=True)
    ...
    if is_superadmin:
        ...
        """, unsafe_allow_html=True)
        # ── Botão criar teste.teste — PRIMEIRO item, sempre visível ──
        _teste_existe = not db_read(
            "SELECT usuario FROM usuarios WHERE usuario='teste.teste'").empty
        if not _teste_existe:
            if st.button("🧪 Criar conta teste.teste", key="sa_criar_teste",
                         use_container_width=True):
                db_exec(
                    "INSERT INTO usuarios "
                    "(usuario,senha,tipo,primeiro_acesso,telefone,aniversario) "
                    "VALUES(?,?,?,?,?,?) ON CONFLICT (usuario) DO NOTHING",
                    ("teste.teste","elkam","teste",1,"",""))
                st.success("🧪 teste.teste criado! Senha: elkam")
                st.rerun()
        else:
            st.markdown(
                "<div style='background:#1a1400;border:1px solid #f59e0b55;"
                "border-radius:8px;padding:6px 10px;font-size:11px;"
                "color:#f59e0b;text-align:center;margin-bottom:6px'>"
                "🧪 teste.teste ativo · senha: elkam</div>",
                unsafe_allow_html=True)
        ...
        if modo_atual == "funcionario":
            funcs_sa = db_read(
                ...
                "WHERE tipo IN ('funcionario','teste') AND usuario!='superadmin'")
            if not funcs_sa.empty:
                func_opts = funcs_sa["usuario"].tolist()
                func_sim  = st.selectbox("👤 Simular funcionário", func_opts,
                                         key="superadmin_func_sim")
                st.session_state["superadmin_simular"] = func_sim
                _tipo_sim = funcs_sa[funcs_sa["usuario"]==func_sim]["tipo"].values
                if len(_tipo_sim) and _tipo_sim[0] == "teste":
                    st.markdown(
                        "<div style='background:#1a1400;border:1px solid #f59e0b55;"
                        "border-radius:8px;padding:8px 12px;font-size:11px;"
                        "color:#f59e0b;margin-top:4px'>"
                        "🧪 Conta TESTE — atividades visíveis<br>apenas ao Superadmin</div>",
                        unsafe_allow_html=True)
            else:
...
if tipo_efetivo == "admin":
    ...
    _MENU_OPTS = ["Dashboard", "Funcionários", "Mercados", "Agenda", "Relatórios",
                  ...
                  "Clientes", "Auditoria", "Configurações"]
    ...
    with st.sidebar:
        ...
        _icons = {"Dashboard":"📊","Funcionários":"👥","Mercados":"🏪","Agenda":"📋",
                  ...
                  "Empresa":"🏢","Clientes":"👥","Auditoria":"🔍","Configurações":"⚙️"}
        # Conta relatórios não vistos para badge no Dashboard
        _n_notif_adm = 0
        try:
            _nr_cnt = db_read(
                "SELECT COUNT(*) as t FROM relatorio WHERE notif_admin=1 AND data=? "
                "AND funcionario NOT IN (SELECT usuario FROM usuarios WHERE tipo='teste')",
                (str(today_br()),))
            _n_notif_adm = int(_nr_cnt.iloc[0]["t"]) if not _nr_cnt.empty else 0
        except: pass
        for _opt in _MENU_OPTS:
            _ico = _icons.get(_opt, "•")
            _ativo = menu == _opt
            _badge_adm = f"  🔔{_n_notif_adm}" if _opt == "Dashboard" and _n_notif_adm > 0 else ""
            _lbl = f"{_ico}  {_opt}{_badge_adm}"
    ...
    if menu == "Dashboard":
        ...
        _notif_rel = db_read(
            "SELECT DISTINCT funcionario, mercado, data, hora FROM relatorio "
            "WHERE notif_admin=1 AND data=? "
            "AND funcionario NOT IN (SELECT usuario FROM usuarios WHERE tipo='teste') "
            "ORDER BY hora DESC",
            (str(_hd),))
        if not _notif_rel.empty:
            for _, _nr in _notif_rel.iterrows():
                _nr_nome = _nr["funcionario"].split(".")[0].capitalize()
                _nr_hora = str(_nr.get("hora","") or "")
                st.markdown(
                    f"<div style='background:#0a1a2b;border:1.5px solid #3b82f633;"
                    f"border-left:4px solid #3b82f6;border-radius:10px;"
                    f"padding:12px 16px;margin:4px 0;display:flex;"
                    f"justify-content:space-between;align-items:center'>"
                    f"<div>"
                    f"<span style='color:#3b82f6;font-size:12px;font-weight:700'>🔔 Novo relatório</span><br>"
                    f"<span style='color:#ddd;font-size:13px'>🏪 {_nr['mercado']}  ·  👤 {_nr_nome}"
                    f"{'  ·  🕐 '+_nr_hora if _nr_hora else ''}</span></div></div>",
                    unsafe_allow_html=True)
            if st.button("✔️ Marcar todos como vistos", key="notif_clear", use_container_width=False):
                db_exec(
                    "UPDATE relatorio SET notif_admin=0 WHERE notif_admin=1 AND data=? "
                    "AND funcionario NOT IN (SELECT usuario FROM usuarios WHERE tipo='teste')",
                    (str(_hd),))
                st.rerun()
            st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        ...
        _ra = db_read("SELECT * FROM relatorio WHERE funcionario NOT IN (SELECT usuario FROM usuarios WHERE tipo='teste')")
        _ca = db_read("SELECT * FROM checkin WHERE funcionario NOT IN (SELECT usuario FROM usuarios WHERE tipo='teste')")
        ...
        _cia = db_read("SELECT * FROM checkin WHERE data=? AND status='em_visita' AND funcionario NOT IN (SELECT usuario FROM usuarios WHERE tipo='teste')", (_hd,))
        _cic = db_read("SELECT * FROM checkin WHERE data=? AND status='concluido' AND funcionario NOT IN (SELECT usuario FROM usuarios WHERE tipo='teste')", (_hd,))
        ...
        _ta = len(_rs) if not _rs.empty else 0
        _tb = len(_rant) if not _rant.empty else 0
        ...
        with _rk_cols[1]:
            ...
            _rall2 = db_read("SELECT * FROM relatorio WHERE status='falta' AND produto_faltante!='' AND produto_faltante IS NOT NULL AND funcionario NOT IN (SELECT usuario FROM usuarios WHERE tipo='teste')")
            if not _rall2.empty:
    ...
    # ── FUNCIONÁRIOS ──────────────────────────────────────
        ...
        funcs = db_read("SELECT usuario, telefone, aniversario, tipo FROM usuarios WHERE tipo IN ('funcionario','teste') AND usuario!='superadmin'")
        ...
        if funcs.empty:
            ...
            for _, row in funcs.iterrows():
                ...
                _is_teste_func = (str(row.get("tipo","")) == "teste")
                _teste_badge   = (" <span style='background:#1a1400;border:1px solid #f59e0b55;"
                                   "border-radius:5px;padding:1px 6px;color:#f59e0b;"
                                   "font-size:10px;font-weight:700'>🧪 TESTE</span>"
                                   if _is_teste_func else "")
                with col_i:
                    st.markdown(
                        f"<div style='color:#ddd;font-size:13px;font-weight:600;padding:10px 0'>"
                        f"👤 {login_func}{_teste_badge}"
                ...
                if st.session_state.get(f"confirm_del_func_{login_func}", False):
                    if _is_teste_func:
                        st.warning(f"🧪 Excluir TESTE **{login_func}**? **Tudo** será apagado permanentemente (relatórios inclusos).")
                    else:
                        st.warning(f"⚠️ Excluir **{login_func}**? Agenda e chat serão removidos. Relatórios mantidos como *(ex func)*.")
                    _cd1, _cd2 = st.columns(2)
                    with _cd1:
                        if st.button("✅ Confirmar exclusão", key=f"ok_del_{login_func}", use_container_width=True):
                            fazer_backup()
                            if _is_teste_func:
                                # Conta de teste: apaga TUDO sem deixar rastro
                                for _tbl in ["usuarios","agenda","checkin","chat","relatorio","auditoria"]:
                                    db_exec(f"DELETE FROM {_tbl} WHERE funcionario=?", (login_func,))
                                db_exec("DELETE FROM usuarios WHERE usuario=?", (login_func,))
                                st.session_state.pop(f"confirm_del_func_{login_func}", None)
                                st.success(f"🧪 Conta de teste **{login_func}** excluída — todos os dados apagados.")
                            else:
                                # Funcionário real: preserva relatórios
    ...
    # ── MERCADOS ──────────────────────────────────────────
        ...
        if st.session_state["exp_novo_merc"]:
            ...
            # Vincular clientes
            _clis_disp_nm = db_read("SELECT id, nome FROM clientes WHERE ativo=1 ORDER BY nome")
            _opts_clis_nm = _clis_disp_nm["nome"].tolist() if not _clis_disp_nm.empty else []
            _clis_vinc_new = st.multiselect(
                "👤 Vincular clientes (podem ser vários)",
                _opts_clis_nm, key="nm_clis_vinc",
                placeholder="Selecione um ou mais clientes...")
            st.caption("📌 Os clientes selecionados terão acesso às métricas deste mercado.")
                ...
                if not mercado.strip() or not endereco.strip():
                    ...
                    if dup:
                        ...
                        try:
                            ...
                            # Vincula clientes selecionados a este mercado
                            for _cl_nm_v in _clis_vinc_new:
                                _cl_row_v = db_read("SELECT id, mercados FROM clientes WHERE nome=? AND ativo=1", (_cl_nm_v,))
                                if not _cl_row_v.empty:
                                    _mercs_atuais_v = [m.strip() for m in str(_cl_row_v.iloc[0]["mercados"] or "").split(",") if m.strip()]
                                    if mercado.strip() not in _mercs_atuais_v:
                                        _mercs_atuais_v.append(mercado.strip())
                                    db_exec("UPDATE clientes SET mercados=? WHERE id=?",
                                              (",".join(_mercs_atuais_v), int(_cl_row_v.iloc[0]["id"])))
        ...
        _todos_clis_lista = db_read("SELECT nome, mercados FROM clientes WHERE ativo=1")

        if mdf.empty:
            st.info("Nenhum mercado cadastrado.")
        else:
            for _, row in mdf.iterrows():
                # Clientes vinculados a este mercado
                _clis_vinc_card = []
                if not _todos_clis_lista.empty:
                    for _, _cr in _todos_clis_lista.iterrows():
                        _cr_mercs = [m.strip() for m in str(_cr.get("mercados","") or "").split(",") if m.strip()]
                        if row["mercado"] in _cr_mercs or not _cr_mercs:
                            _clis_vinc_card.append(str(_cr.get("nome","")))
                _tag_clis = (
                    f" <span style='color:#3b82f6;font-size:11px'>· 👤 {', '.join(_clis_vinc_card)}</span>"
                    if _clis_vinc_card else
                    " <span style='color:#333;font-size:11px'>· Sem cliente vinculado</span>"
                )
                ...
                with col_i:
                    st.markdown(
                        ...
                        f" <span style='color:#666'>{row['endereco']}</span>"
                        f"{_tag_clis}</div>",
                        unsafe_allow_html=True)
                with col_ed:
                ...
                if st.session_state.get(f"editando_{row['id']}", False):
                    with st.container():
                        ...
                        # Clientes atualmente vinculados a este mercado
                        _clis_edit_all = db_read("SELECT id, nome, mercados FROM clientes WHERE ativo=1 ORDER BY nome")
                        _opts_clis_ed = _clis_edit_all["nome"].tolist() if not _clis_edit_all.empty else []
                        _vinc_atuais_ed = []
                        if not _clis_edit_all.empty:
                            for _, _ce_r in _clis_edit_all.iterrows():
                                _ce_mercs = [m.strip() for m in str(_ce_r.get("mercados","") or "").split(",") if m.strip()]
                                if row["mercado"] in _ce_mercs:
                                    _vinc_atuais_ed.append(str(_ce_r["nome"]))
                        _ed_clis_sel = st.multiselect(
                            "👤 Clientes vinculados",
                            _opts_clis_ed,
                            default=_vinc_atuais_ed,
                            key=f"edit_clis_{row['id']}",
                            placeholder="Selecione um ou mais clientes...")
                        st.caption("Os clientes selecionados terão acesso às métricas deste mercado.")
                        ...
                        with ec:
                            if st.button("✅ Salvar", key=f"save_merc_{row['id']}"):
                                ...
                                # Atualiza vínculos: remove este mercado de todos os clientes e re-adiciona apenas nos selecionados
                                if not _clis_edit_all.empty:
                                    for _, _cu_r in _clis_edit_all.iterrows():
                                        _cu_mercs = [m.strip() for m in str(_cu_r.get("mercados","") or "").split(",") if m.strip()]
                                        _nome_merc_novo = novo_nome.strip()
                                        # remove referência ao nome antigo e ao nome novo (para evitar duplicata)
                                        _cu_mercs = [m for m in _cu_mercs if m not in (nome_antigo, _nome_merc_novo)]
                                        if str(_cu_r["nome"]) in _ed_clis_sel:
                                            _cu_mercs.append(_nome_merc_novo)
                                        db_exec("UPDATE clientes SET mercados=? WHERE id=?",
                                                  (",".join(_cu_mercs), int(_cu_r["id"])))
    ...
    # ── AGENDA ────────────────────────────────────────────
        ...
        funcs_ag = db_read("SELECT usuario FROM usuarios WHERE tipo IN ('funcionario','teste') AND usuario!='superadmin'")
        ...
        if st.session_state["exp_add_agenda"]:
            if funcs_ag.empty:
                ...
                lista_prod = sorted(prods_cad["produto"].tolist()) if not prods_cad.empty else []
                    ...
                    if st.button("Adicionar à agenda", key="ag_add"):
                        if not ps.strip():
                            ...
                            _dup_df = db_read("SELECT COUNT(*) as cnt FROM agenda WHERE funcionario=? AND dia=? AND mercado=? AND produto=?",
                                (fs, ds, ms, ps.strip()))
                            dup = int(_dup_df.iloc[0]["cnt"]) if not _dup_df.empty else 0
                            if dup > 0:
                                ...
                                if ps.strip() not in lista_prod:
                                    _dup_p_df = db_read("SELECT COUNT(*) as cnt FROM produtos WHERE produto=?", (ps.strip(),))
                                    dup_p = int(_dup_p_df.iloc[0]["cnt"]) if not _dup_p_df.empty else 0
                                    if dup_p == 0:
        ...
        _all_funcs = db_read("SELECT usuario FROM usuarios WHERE tipo IN ('funcionario','teste') AND usuario!='superadmin'")
        _mercs_list = db_read("SELECT mercado FROM mercados")["mercado"].tolist() if not db_read("SELECT mercado FROM mercados").empty else []
        _prods_list = sorted(db_read("SELECT DISTINCT produto FROM produtos")["produto"].tolist()) if not db_read("SELECT DISTINCT produto FROM produtos").empty else []

        if _all_funcs.empty:
            st.info("Nenhum funcionário cadastrado.")
        else:
            for _func_u in _all_funcs["usuario"].tolist():
                _ag_f = db_read("SELECT id, dia, mercado, produto FROM agenda WHERE funcionario=?", (_func_u,))
                _n_items = len(_ag_f)
                                ...
                                for _, _row in _pgs.iterrows():
                                    ...
                                    with _pc1:
                                        st.markdown(
                                            ...
                                            unsafe_allow_html=True)
                                    with _pc2:
                                        if st.button("🗑️", key=f"del_ag_{_row['id']}_{_func_u}",
                                                     help=f"Remover {_row['produto']}"):
                                            db_exec("DELETE FROM agenda WHERE id=?", (int(_row["id"]),))
                                            st.rerun()
                    ...
                    if st.session_state[_ak]:
                        ...
                        if st.button("✅ Confirmar adição", key=f"ia_add_{_func_u}", use_container_width=True):
                            if not _ps2.strip():
                                ...
                                _dup_df3 = db_read(
                                    "SELECT COUNT(*) as cnt FROM agenda WHERE funcionario=? AND dia=? AND mercado=? AND produto=?",
                                    (_func_u, _ds2, _ms2, _ps2.strip()))
                                _dup = int(_dup_df3.iloc[0]["cnt"]) if not _dup_df3.empty else 0
                                if _dup > 0:
                                    ...
                                    if _ps2.strip() not in _prods_list:
                                        db_exec("INSERT INTO produtos (mercado,produto) VALUES(?,?) ON CONFLICT DO NOTHING",
                                                  (_ms2, _ps2.strip()))
    ...
    # ── RELATÓRIOS ────────────────────────────────────────
        ...
        def _gerar_html_rel(df_rows, titulo="El Kam Merchandising", subtitulo="", apagar_fotos=False):
            ...
            for _mh in sorted(_mercs_h):
                ...
                for _, _rh2 in _df_mh.iterrows():
                    ...
                    _preco_r = str(_rh2.get("preco_valor","") or "")
                    _fp_preco_r = str(_rh2.get("foto_preco","") or "")

                    # Comparativo de preço com ontem
                    _data_ontem_r = ""
                    _preco_delta_html = ""
                    try:
                        _data_ontem_r = (pd.to_datetime(str(_rh2.get("data",""))[:10]) - timedelta(days=1)).strftime("%Y-%m-%d")
                        _row_ontem_r = db_read("SELECT preco_valor FROM relatorio WHERE data=? AND mercado=? AND produto=? AND preco_valor!=''",
                                               (_data_ontem_r, _rh2.get("mercado",""), _prod_nm))
                        if not _row_ontem_r.empty and _preco_r:
                            _pv2 = float(_preco_r.replace(",",".").replace("R$","").strip())
                            _po2 = float(str(_row_ontem_r.iloc[0]["preco_valor"]).replace(",",".").replace("R$","").strip())
                            _diff2 = _pv2 - _po2
                            if _diff2 > 0:
                                _preco_delta_html = f"<span style='color:#dc2626;font-size:11px'>▲ R$ {_diff2:.2f}</span>"
                            elif _diff2 < 0:
                                _preco_delta_html = f"<span style='color:#16a34a;font-size:11px'>▼ R$ {abs(_diff2):.2f}</span>"
                    except: pass

                    # Lê foto do preço como base64 para o relatório
                    _fp_preco_b64 = ""
                    if _fp_preco_r and os.path.exists(_fp_preco_r):
                        try:
                            import base64 as _b64_mod
                            with open(_fp_preco_r, "rb") as _fp_pr_f:
                                _fp_preco_b64 = _b64_mod.b64encode(_fp_pr_f.read()).decode()
                        except: pass
                    ...
                    if _b64_r.strip():
                        ...
                        _aid_preco = f"foto_preco_{_foto_idx}"
                        _fotos_bloco = (
                            ...
                            f"<div><div style='font-size:11px;color:#888;margin-bottom:4px'>📷 Foto do produto</div>"
                            f"<img src='data:image/jpeg;base64,{_b64_r}' style='max-width:280px;max-height:320px;border-radius:8px;border:2px solid #ddd'></div>"
                        )
                        if _fp_preco_b64:
                            _fotos_bloco += (
                                f"<div><div style='font-size:11px;color:#888;margin-bottom:4px'>🏷️ Foto do preço</div>"
                                f"<img src='data:image/jpeg;base64,{_fp_preco_b64}' style='max-width:280px;max-height:320px;border-radius:8px;border:2px solid #3b82f6'></div>"
                            )
                        _fotos_bloco += "</div></div>"
                        _fotos_pasta += _fotos_bloco
                    else:
                        _foto_link = "<span style='color:#aaa;font-size:12px'>Sem foto</span>"

                    # Link da foto do preço (caso não tenha foto principal mas tenha foto do preço)
                    _aid_preco2 = f"foto_preco_solo_{_foto_idx}"
                    if _fp_preco_b64 and not _b64_r.strip():
                        _n_fotos_merc += 1
                        _foto_link_preco = f"<a href='#pasta_{_mh.replace(' ','_')}' onclick='document.getElementById(\"{_aid_preco2}\").style.display=\"block\";return false;' style='color:#3b82f6;font-size:12px;text-decoration:none'>🏷️ Ver preço</a>"
                        _fotos_pasta += (
                            f"<div id='{_aid_preco2}' style='display:none;margin:12px 0;padding:14px;background:#eff6ff;border-radius:10px;border:1px solid #bfdbfe'>"
                            f"<div style='font-weight:700;font-size:13px;margin-bottom:10px;color:#222'>🏷️ {_prod_nm} — Foto do preço</div>"
                            f"<img src='data:image/jpeg;base64,{_fp_preco_b64}' style='max-width:100%;max-height:320px;border-radius:8px;border:2px solid #3b82f6'>"
                            f"</div>"
                        )
                    else:
                        _foto_link_preco = (f"<span style='color:#3b82f6;font-size:12px'>🏷️ (na pasta)</span>"
                                            if _fp_preco_b64 else "<span style='color:#aaa;font-size:12px'>—</span>")
                    ...
                    _prods_rows += (
                        ...
                        f"{'<b style=color:#1a56db>R$ '+_preco_r+'</b>' if _preco_r else '—'}"
                        f"{'  '+_preco_delta_html if _preco_delta_html else ''}</td>"
                        f"<td style='padding:10px 12px'>{_foto_link_preco}</td>"
                        f"<td style='padding:10px 12px'>{_foto_link}</td>"
                        f"</tr>"
                    )
                    if apagar_fotos:
                        db_exec("UPDATE relatorio SET foto_b64='',foto='' WHERE id=?", (int(_rh2.get("id",0)),))
                ...
                _blocos += (
                    ...
                    f"<th style='padding:10px 14px;text-align:left;border-bottom:2px solid #eee;color:#444'>Preço</th>"
                    f"<th style='padding:10px 14px;text-align:left;border-bottom:2px solid #eee;color:#444'>Foto Preço</th>"
                    f"<th style='padding:10px 14px;text-align:left;border-bottom:2px solid #eee;color:#444'>Foto Produto</th>"
        ...
        # Exclui relatórios de contas teste dos relatórios do admin normal
        rel_all = db_read(
            "SELECT id, * FROM relatorio WHERE funcionario NOT IN "
            "(SELECT usuario FROM usuarios WHERE tipo='teste')")
        _mercs_info = db_read("SELECT mercado, telefone_rel, email_rel FROM mercados")

        st.markdown("### 🔍 Filtros")
        # Listas sempre preenchidas do banco (não dependem de rel_all)
        _mercs_f = sorted(db_read("SELECT DISTINCT mercado FROM mercados ORDER BY mercado")["mercado"].tolist()
                          if not db_read("SELECT 1 FROM mercados LIMIT 1").empty else [])
        _funcs_f = sorted(db_read(
            "SELECT usuario FROM usuarios WHERE tipo='funcionario' "
            "AND usuario!='superadmin' ORDER BY usuario")["usuario"].tolist()
                          if not db_read("SELECT 1 FROM usuarios WHERE tipo='funcionario'").empty else [])
        # Complementa com valores que estão nos relatórios
        if not rel_all.empty:
            _mercs_f = sorted(set(_mercs_f) | set(rel_all["mercado"].dropna().unique()))
            _funcs_f = sorted(set(_funcs_f) | set(rel_all["funcionario"].dropna().unique()))
        fc1, fc2, fc3 = st.columns(3)
        with fc1:
            periodo = st.selectbox("Período", ["Hoje","Últimos 7 dias","Últimos 30 dias","Intervalo personalizado"], index=0)
        with fc2:
            fm_rel = st.selectbox("Mercado", ["Todos"] + _mercs_f)
        with fc3:
            ff_rel = st.selectbox("Funcionário", ["Todos"] + _funcs_f)
        ...
        # ── SUPERADMIN: relatórios de teste separados ──
        if is_superadmin:
            _rel_teste = db_read(
                "SELECT id,* FROM relatorio WHERE funcionario IN "
                "(SELECT usuario FROM usuarios WHERE tipo='teste')")
            if not _rel_teste.empty:
                _rel_teste["data_dt"] = pd.to_datetime(_rel_teste["data"], errors="coerce")
                _rel_teste_p = _rel_teste[
                    (_rel_teste["data_dt"].dt.date >= d_ini) &
                    (_rel_teste["data_dt"].dt.date <= d_fim)]
                if not _rel_teste_p.empty:
                    st.markdown(
                        "<div style='background:#1a1400;border:1px solid #f59e0b66;"
                        "border-radius:12px;padding:14px 18px;margin:12px 0'>"
                        "<div style='color:#f59e0b;font-size:12px;font-weight:700;"
                        "letter-spacing:1px;margin-bottom:10px'>"
                        "🧪 SIMULAÇÕES — Visível apenas ao Superadmin</div>",
                        unsafe_allow_html=True)
                    for _, _rt in _rel_teste_p.iterrows():
                        _badge_t = {"abastecido":"✅","falta":"⚠️","fechado":"❌"}.get(
                            str(_rt.get("status","")), "•")
                        _foto_t = _rt.get("foto_b64","") or ""
                        st.markdown(
                            f"<div style='background:#111;border:1px solid #f59e0b22;"
                            f"border-radius:10px;padding:10px 14px;margin:4px 0;"
                            f"font-size:12px;color:#888'>"
                            f"🧪 {_badge_t} <b style='color:#f59e0b'>{_rt.get('funcionario','')}</b>"
                            f" · {_rt.get('mercado','')} · <i>{_rt.get('produto','')}</i>"
                            f" · {_rt.get('data','')} {_rt.get('hora','')}"
                            f"</div>", unsafe_allow_html=True)
                        if _foto_t and len(_foto_t) > 20:
                            import base64 as _b64t
                            st.image(f"data:image/jpeg;base64,{_foto_t}",
                                     width=200, caption="📸 Foto do teste")
                    st.markdown("</div><hr style='border-color:#1c1c1c;margin:16px 0'>",
                                unsafe_allow_html=True)
        ...
        if df_r.empty:
            ...
            for _merc_r in sorted(_mercs_rel):
                ...
                with _ck_r:
                    # Seleciona todos os ids deste mercado de uma vez
                    if st.checkbox("", key=f"sel_merc_{_merc_r}", label_visibility="collapsed"):
                        for _rid_r in _df_merc_r["id"].tolist():
            ...
            with _a1:
                if st.button("🗑️ Excluir selecionados", key="del_rel", disabled=not selecionados_rel, use_container_width=True):
                    ...
                    for rid in selecionados_rel:
                        db_exec("DELETE FROM relatorio WHERE id=?", (rid,))
                    st.success("Excluído(s).")
                    st.rerun()
            with _a2:
                if st.button("📥 Baixar relatório (com fotos)", key="dl_rel", use_container_width=True):
                    _df_dl = df_r[df_r["id"].isin(selecionados_rel)] if selecionados_rel else df_r
                    ...
                    st.download_button("⬇️ Clique para baixar", data=_html_dl.encode("utf-8"),
                        ...
                        key="dl_btn_rel")
            with _a3:
                if st.button("👤 Enviar ao cliente", key="enviar_merc_btn",
                             disabled=not selecionados_rel, use_container_width=True):
                    st.session_state["confirm_envio_merc"] = True
                    st.rerun()

            # ── PAINEL DE ENVIO AO CLIENTE ──
            if st.session_state.get("confirm_envio_merc", False):
                _df_env = df_r[df_r["id"].isin(selecionados_rel)]
                _merc_env = _df_env["mercado"].unique().tolist() if not _df_env.empty else []

                # Busca todos os clientes vinculados aos mercados selecionados
                _todos_clientes = db_read("SELECT nome, email, telefone, mercados FROM clientes WHERE ativo=1")
                _clientes_envio = []  # lista de dicts {nome, email, telefone, mercados}
                if not _todos_clientes.empty:
                    for _, _cl_row in _todos_clientes.iterrows():
                        _cl_mercs_str = str(_cl_row.get("mercados","") or "")
                        # vazio = todos os mercados; senão verifica interseção
                        _cl_mercs_lista = [m.strip() for m in _cl_mercs_str.split(",") if m.strip()]
                        _vinculado = (not _cl_mercs_lista) or any(m in _merc_env for m in _cl_mercs_lista)
                        if _vinculado:
                            _clientes_envio.append({
                                "nome": str(_cl_row.get("nome","") or ""),
                                "email": str(_cl_row.get("email","") or ""),
                                "telefone": str(_cl_row.get("telefone","") or ""),
                                "mercados": _cl_mercs_lista or ["Todos"],
                            })

                # Cabeçalho do painel
                st.markdown(
                    "<div style='background:#0a0f1a;border:2px solid #3b82f644;border-radius:14px;"
                    "padding:20px;margin:12px 0'>",
                    unsafe_allow_html=True)
                st.markdown(
                    f"<div style='color:#3b82f6;font-size:15px;font-weight:800;margin-bottom:4px'>"
                    f"👤 Enviar relatório ao cliente</div>"
                    f"<div style='color:#555;font-size:12px;margin-bottom:16px'>"
                    f"Mercados selecionados: <b style='color:#ccc'>{', '.join(_merc_env)}</b></div>",
                    unsafe_allow_html=True)

                # ── PASSO A PASSO VISUAL ──
                st.markdown(
                    "<div style='background:#0d1a0d;border:1px solid #22c55e33;border-radius:10px;"
                    "padding:14px 16px;margin-bottom:16px'>"
                    "<div style='color:#22c55e;font-size:11px;font-weight:700;letter-spacing:1px;"
                    "text-transform:uppercase;margin-bottom:10px'>📋 Como enviar o relatório</div>"
                    "<div style='display:flex;flex-direction:column;gap:8px'>"
                    "<div style='display:flex;align-items:flex-start;gap:10px'>"
                    "<span style='background:#22c55e;color:#000;border-radius:50%;width:20px;height:20px;"
                    "display:flex;align-items:center;justify-content:center;font-size:11px;"
                    "font-weight:800;flex-shrink:0'>1</span>"
                    "<span style='color:#ccc;font-size:13px'>Clique em <b style='color:#22c55e'>⬇️ Baixar relatório</b> "
                    "para salvar o arquivo <b>.html</b> no seu dispositivo</span></div>"
                    "<div style='display:flex;align-items:flex-start;gap:10px'>"
                    "<span style='background:#25D366;color:#000;border-radius:50%;width:20px;height:20px;"
                    "display:flex;align-items:center;justify-content:center;font-size:11px;"
                    "font-weight:800;flex-shrink:0'>2</span>"
                    "<span style='color:#ccc;font-size:13px'>Abra o <b style='color:#25D366'>WhatsApp</b> ou "
                    "<b style='color:#1a56db'>Email</b> do cliente clicando no botão abaixo</span></div>"
                    "<div style='display:flex;align-items:flex-start;gap:10px'>"
                    "<span style='background:#3b82f6;color:#000;border-radius:50%;width:20px;height:20px;"
                    "display:flex;align-items:center;justify-content:center;font-size:11px;"
                    "font-weight:800;flex-shrink:0'>3</span>"
                    "<span style='color:#ccc;font-size:13px'>"
                    "No WhatsApp/Email, <b style='color:#f59e0b'>📎 anexe o arquivo</b> baixado antes de enviar</span></div>"
                    "</div></div>",
                    unsafe_allow_html=True)

                # ── BOTÃO DE DOWNLOAD DESTACADO ──
                _html_env_gerado = _gerar_html_rel(
                    _df_env,
                    f"Relatório El Kam — {', '.join(_merc_env)}",
                    f"Gerado em {now_br().strftime('%d/%m/%Y %H:%M')}",
                    apagar_fotos=False)
                _fname_env = f"relatorio_elkam_{d_ini}_{d_fim}.html"

                _dl_col, _ok_col, _cancel_col = st.columns(3)
                with _dl_col:
                    st.download_button(
                        "⬇️ Baixar relatório (.html)",
                        data=_html_env_gerado.encode("utf-8"),
                        file_name=_fname_env,
                        mime="text/html",
                        key="dl_e_envia",
                        use_container_width=True)
                with _ok_col:
                    if st.button("✅ Marcar como enviado", key="so_envia", use_container_width=True):
                        for rid in selecionados_rel:
                            db_exec("UPDATE relatorio SET enviado_mercado=1 WHERE id=?", (rid,))
                        st.session_state["confirm_envio_merc"] = False
                        st.success("✅ Relatório marcado como enviado!")
                        st.rerun()
                with _cancel_col:
                ...
                st.markdown("<hr style='border-color:#1c1c2e;margin:16px 0'>", unsafe_allow_html=True)

                # ── CLIENTES VINCULADOS ──
                if not _clientes_envio:
                    st.markdown(
                        "<div style='color:#555;font-size:13px;padding:12px 0'>"
                        "⚠️ Nenhum cliente vinculado a esses mercados. Cadastre clientes na aba <b>Clientes</b>.</div>",
                        unsafe_allow_html=True)
                else:
                    st.markdown(
                        f"<div style='color:#888;font-size:11px;font-weight:700;letter-spacing:1px;"
                        f"text-transform:uppercase;margin-bottom:12px'>"
                        f"👥 {len(_clientes_envio)} cliente(s) — abra o contato e anexe o arquivo baixado</div>",
                        unsafe_allow_html=True)
                    for _cl_e in _clientes_envio:
                        _cl_nome_e  = _cl_e["nome"]
                        _cl_tel_e   = _cl_e["telefone"].replace(" ","").replace("-","").replace("(","").replace(")","")
                        _cl_email_e = _cl_e["email"]
                        _cl_mercs_e = ", ".join(_cl_e["mercados"])
                        # Monta mensagem WhatsApp (texto de acompanhamento — arquivo vai na mão)
                        _n_vis_cl = len(_df_env)
                        _n_falta_cl = len(_df_env[_df_env["status"]=="falta"]) if "status" in _df_env.columns else 0
                        _msg_cl = _upr.quote(
                            f"Olá, {_cl_nome_e}! 👋\n\n"
                            f"📋 *Relatório El Kam Merchandising*\n"
                            f"🏪 {', '.join(_merc_env)}\n"
                            f"📅 {d_ini.strftime('%d/%m/%Y')} a {d_fim.strftime('%d/%m/%Y')}\n"
                            f"✅ Visitas: {_n_vis_cl}  |  ⚠️ Faltas: {_n_falta_cl}\n\n"
                        ...
                        st.markdown(
                            ...
                            f"<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:10px'>"
                            f"<span style='color:#fff;font-size:14px;font-weight:700'>👤 {_cl_nome_e}</span>"
                            f"<span style='background:#1c2333;color:#3b82f6;font-size:11px;padding:3px 8px;"
                            f"border-radius:6px'>🏪 {_cl_mercs_e}</span></div>"
                            f"<div style='color:#f59e0b;font-size:11px;margin-bottom:10px;'>"
                            f"📎 Lembre-se de anexar o arquivo .html antes de enviar!</div>",
                            unsafe_allow_html=True)
                        _btn_cols = st.columns(3)
                        with _btn_cols[0]:
                            if _cl_tel_e:
                                st.markdown(
                                    f"<a href='https://wa.me/55{_cl_tel_e}?text={_msg_cl}' target='_blank'>"
                                    f"<button style='background:#25D366;color:#fff;border:none;border-radius:8px;"
                                    f"padding:9px 12px;font-size:12px;font-weight:700;cursor:pointer;width:100%'>"
                                    f"📲 WhatsApp Celular</button></a>",
                                    unsafe_allow_html=True)
                            else:
                                st.markdown("<div style='color:#333;font-size:11px;padding:8px'>Sem telefone</div>", unsafe_allow_html=True)
                        with _btn_cols[1]:
                            if _cl_tel_e:
                                st.markdown(
                                    f"<a href='https://web.whatsapp.com/send?phone=55{_cl_tel_e}&text={_msg_cl}' target='_blank'>"
                                    f"<button style='background:#128C7E;color:#fff;border:none;border-radius:8px;"
                                    f"padding:9px 12px;font-size:12px;font-weight:700;cursor:pointer;width:100%'>"
                                    f"💻 WhatsApp PC</button></a>",
                                    unsafe_allow_html=True)
                            else:
                                st.markdown("<div style='color:#333;font-size:11px;padding:8px'>&nbsp;</div>", unsafe_allow_html=True)
                        with _btn_cols[2]:
                            if _cl_email_e:
                                _subj_cl = _upr.quote(f"Relatório El Kam — {', '.join(_merc_env)} — {d_ini.strftime('%d/%m/%Y')}")
                                _body_cl = _upr.quote(
                                    f"Olá, {_cl_nome_e}!\n\n"
                                    f"Segue o relatório de merchandising de {', '.join(_merc_env)}.\n"
                                    f"Período: {d_ini.strftime('%d/%m/%Y')} a {d_fim.strftime('%d/%m/%Y')}.\n\n"
                                    f"Att,\nEl Kam Merchandising")
                                st.markdown(
                                    f"<a href='mailto:{_cl_email_e}?subject={_subj_cl}&body={_body_cl}' target='_blank'>"
                                    f"<button style='background:#1a56db;color:#fff;border:none;border-radius:8px;"
                                    f"padding:9px 12px;font-size:12px;font-weight:700;cursor:pointer;width:100%'>"
                                    f"📧 Email</button></a>",
                                    unsafe_allow_html=True)
                            else:
                                st.markdown("<div style='color:#333;font-size:11px;padding:8px'>Sem email</div>", unsafe_allow_html=True)
                        st.markdown("</div>", unsafe_allow_html=True)
    ...
    # ── FOTOS DE PRODUTOS ─────────────────────────────────
        ...
        if st.session_state["exp_foto_prod"]:
            ...
            with pf1:
                ...
                lista_p = sorted(prods_todos["produto"].tolist()) if not prods_todos.empty else []
            ...
            if st.button("Salvar foto do produto", key="pf_save"):
                if not p_nome_foto.strip():
                    ...
                    # Upsert
                    _ex_df = db_read("SELECT id FROM produto_fotos WHERE produto=?", (p_nome_foto.strip(),))
                    ex = tuple(_ex_df.iloc[0]) if not _ex_df.empty else None
                    if ex:
    ...
    # ── CHAT ADMIN ────────────────────────────────────────
        ...
        funcs_ch = db_read("SELECT usuario, tipo FROM usuarios WHERE tipo IN ('funcionario','teste') AND usuario!='superadmin'")
    ...
    # ── DESTINATÁRIOS PDF ─────────────────────────────────
        ...
        page_header("Gestão", "Clientes")
        st.caption("Cadastre clientes com login próprio para visualizarem os relatórios.")

        # ── CADASTRO NOVO CLIENTE (toggle, sem sobreposição) ──
        if "exp_cad_cli" not in st.session_state:
            st.session_state["exp_cad_cli"] = False
        _lbl_cli_btn = "🔼 Fechar formulário" if st.session_state["exp_cad_cli"] else "➕ Novo cliente"
        if st.button(_lbl_cli_btn, key="btn_exp_cad_cli"):
            st.session_state["exp_cad_cli"] = not st.session_state["exp_cad_cli"]
            st.rerun()

        if st.session_state["exp_cad_cli"]:
            st.markdown(
                "<div style='background:#0d1a0d;border:1.5px solid #ff2b2b44;border-radius:14px;"
                "padding:18px 20px;margin:8px 0 16px'>",
                unsafe_allow_html=True)
            _c1, _c2 = st.columns(2)
            with _c1: _cl_nome  = st.text_input("Nome completo", key="cl_nm")
            with _c2: _cl_email = st.text_input("E-mail", key="cl_em")
            _c3, _c4 = st.columns(2)
            with _c3: _cl_user  = st.text_input("Login (usuário)", key="cl_us", placeholder="empresa.cliente")
            with _c4: _cl_tel   = st.text_input("📱 WhatsApp (com DDD, só números)", key="cl_tel", placeholder="11999998888")
            _all_mercs_cl = db_read("SELECT mercado FROM mercados ORDER BY mercado")
            _opts_mercs_cl = _all_mercs_cl["mercado"].tolist() if not _all_mercs_cl.empty else []
            _cl_mercs_sel = st.multiselect("Mercados visíveis (vazio = todos)", _opts_mercs_cl, key="cl_ms")
            st.caption("🔐 Senha padrão: **elkam** · O cliente troca no 1º acesso")

            # Botões de envio de login via WhatsApp
            if _cl_tel:
                import urllib.parse as _up_cli
                _link_app = "https://elkam-merchandising.streamlit.app"
                _msg_cli_wa = (
                    f"Olá, {_cl_nome or 'cliente'}! 👋\n\n"
                    f"Seu acesso ao portal El Kam Merchandising está pronto.\n\n"
                    f"🔗 *Link:* {_link_app}\n"
                    f"👤 *Usuário:* {_cl_user or '(defina o login)'}\n"
                    f"🔑 *Senha:* elkam _(troque no primeiro acesso)_\n\n"
                    f"Qualquer dúvida, entre em contato conosco!"
                )
                _enc_cli = _up_cli.quote(_msg_cli_wa)
                _wa_cel  = f"https://wa.me/55{_cl_tel}?text={_enc_cli}"
                _wa_pc   = f"https://web.whatsapp.com/send?phone=55{_cl_tel}&text={_enc_cli}"
                st.markdown(
                    f"<div style='margin:8px 0 4px;color:#25D366;font-size:11px;font-weight:700'>📲 Enviar login por WhatsApp após cadastrar:</div>"
                    f"<div style='display:flex;gap:8px;margin-bottom:8px'>"
                    f"<a href='{_wa_cel}' target='_blank' style='flex:1'>"
                    f"<button style='background:#25D366;color:#fff;border:none;border-radius:8px;"
                    f"padding:9px;font-size:12px;font-weight:700;cursor:pointer;width:100%'>📲 Celular</button></a>"
                    f"<a href='{_wa_pc}' target='_blank' style='flex:1'>"
                    f"<button style='background:#128C7E;color:#fff;border:none;border-radius:8px;"
                    f"padding:9px;font-size:12px;font-weight:700;cursor:pointer;width:100%'>💻 PC</button></a>"
                    f"</div>",
                    unsafe_allow_html=True)

            if st.button("💾 Cadastrar cliente", key="cl_add"):
                if not _cl_nome.strip() or not _cl_user.strip():
                    st.error("Nome e login são obrigatórios.")
                elif not db_read("SELECT id FROM clientes WHERE LOWER(usuario)=LOWER(?)", (_cl_user.strip(),)).empty:
                    st.error("Este login já existe.")
                else:
                    db_exec("INSERT INTO clientes (nome,email,usuario,senha,mercados,ativo,telefone,primeiro_acesso) VALUES(?,?,?,?,?,1,?,1)",
                            (_cl_nome.strip(), _cl_email.strip(), _cl_user.strip(),
                             "elkam", ",".join(_cl_mercs_sel), _cl_tel.strip()))
                    st.success(f"✅ Cliente **{_cl_nome.strip()}** cadastrado! Login: `{_cl_user.strip()}` · Senha padrão: elkam")
                    st.session_state["exp_cad_cli"] = False
                    st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)

        section_title("Clientes cadastrados")
        _cls = db_read("SELECT * FROM clientes ORDER BY nome")
        if _cls.empty:
            st.info("Nenhum cliente cadastrado ainda.")
        else:
            for _, _cl in _cls.iterrows():
                _cl_id   = int(_cl["id"])
                _cl_user_d = str(_cl.get("usuario",""))
                _cl_nome_d = str(_cl.get("nome",""))
                _cl_tel_d  = str(_cl.get("telefone","") or "")
                _cl_mercs_d = str(_cl.get("mercados","") or "Todos os mercados")
                _cl_ativo_d = _cl.get("ativo", 1)
                _cl_pa_d    = _cl.get("primeiro_acesso", 0)

                # ── Card do cliente ──
                _s_txt = "🟢 Ativo" if _cl_ativo_d else "🔴 Inativo"
                _pa_tag = " · <span style='color:#f59e0b;font-size:10px'>⏳ Aguardando 1º acesso</span>" if _cl_pa_d else ""
                st.markdown(
                    f"<div style='background:#111;border:1px solid #222;border-radius:10px;"
                    f"padding:12px 16px;margin:5px 0'>"
                    f"<span style='color:#fff;font-weight:700;font-size:14px'>{_cl_nome_d}</span>"
                    f"<span style='color:#555;font-size:12px'> · {_s_txt}{_pa_tag}</span><br>"
                    f"<span style='color:#444;font-size:11px'>🔑 {_cl_user_d} · 📧 {_cl.get('email','') or '—'}"
                    f" · {'📱 '+_cl_tel_d if _cl_tel_d else 'sem telefone'}</span><br>"
                    f"<span style='color:#333;font-size:11px'>🏪 {_cl_mercs_d}</span></div>",
                    unsafe_allow_html=True)

                # Linha de ações
                _ca1, _ca2, _ca3, _ca4, _ca5 = st.columns([2, 2, 1, 1, 1])

                # ── Enviar login por WA ──
                with _ca1:
                    if _cl_tel_d:
                        import urllib.parse as _up_cl2
                        _link_app2 = "https://elkam-merchandising.streamlit.app"
                        _msg_wa2 = (
                            f"Olá, {_cl_nome_d}! 👋\n\n"
                            f"Acesse o portal El Kam Merchandising:\n\n"
                            f"🔗 *Link:* {_link_app2}\n"
                            f"👤 *Usuário:* {_cl_user_d}\n"
                            f"🔑 *Senha:* elkam _(troque no primeiro acesso)_"
                        )
                        _enc2 = _up_cl2.quote(_msg_wa2)
                        st.markdown(
                            f"<div style='display:flex;gap:5px;padding:2px 0'>"
                            f"<a href='https://wa.me/55{_cl_tel_d}?text={_enc2}' target='_blank' style='flex:1'>"
                            f"<button style='background:#25D366;color:#fff;border:none;border-radius:7px;"
                            f"padding:7px 6px;font-size:10px;font-weight:700;cursor:pointer;width:100%'>📲 WA Cel.</button></a>"
                            f"<a href='https://web.whatsapp.com/send?phone=55{_cl_tel_d}&text={_enc2}' target='_blank' style='flex:1'>"
                            f"<button style='background:#128C7E;color:#fff;border:none;border-radius:7px;"
                            f"padding:7px 6px;font-size:10px;font-weight:700;cursor:pointer;width:100%'>💻 WA PC</button></a>"
                            f"</div>",
                            unsafe_allow_html=True)
                    else:
                ...
                # ── Resetar senha ──
                with _ca2:
                    if st.button("🔑 Resetar senha", key=f"cl_reset_{_cl_id}"):
                        db_exec("UPDATE clientes SET senha='elkam', primeiro_acesso=1 WHERE id=?", (_cl_id,))
                        st.success(f"Senha de **{_cl_nome_d}** resetada para **elkam**.")
                        st.rerun()

                # ── Editar ──
                with _ca3:
                    if st.button("✏️", key=f"cl_edit_{_cl_id}", help="Editar cliente"):
                        _ek_cli = f"editando_cli_{_cl_id}"
                        st.session_state[_ek_cli] = not st.session_state.get(_ek_cli, False)
                        for _k2 in list(st.session_state.keys()):
                            if _k2.startswith("editando_cli_") and _k2 != _ek_cli:
                                st.session_state[_k2] = False
                        st.rerun()

                # ── Excluir ──
                with _ca4:
                    if st.button("🗑️", key=f"cl_del_btn_{_cl_id}", help="Excluir cliente"):
                        st.session_state[f"confirm_del_cli_{_cl_id}"] = True
                        st.rerun()

                # ── Confirmação de exclusão ──
                if st.session_state.get(f"confirm_del_cli_{_cl_id}", False):
                    st.markdown(
                        f"<div style='background:#1a0000;border:1.5px solid #ff2b2b44;border-radius:10px;"
                        f"padding:14px 18px;margin:4px 0'>",
                        unsafe_allow_html=True)
                    st.warning(f"⚠️ Deseja excluir o cliente **{_cl_nome_d}**?")
                    _cd1, _cd2, _cd3 = st.columns(3)
                    with _cd1:
                        if st.button("✅ Sim, excluir", key=f"cl_del_conf_{_cl_id}"):
                            db_exec("DELETE FROM clientes WHERE id=?", (_cl_id,))
                            st.session_state.pop(f"confirm_del_cli_{_cl_id}", None)
                            st.rerun()
                    with _cd2:
                        if st.button("✏️ Editar em vez disso", key=f"cl_del_edit_{_cl_id}"):
                            st.session_state[f"confirm_del_cli_{_cl_id}"] = False
                            st.session_state[f"editando_cli_{_cl_id}"] = True
                            st.rerun()
                    with _cd3:
                        if st.button("❌ Cancelar", key=f"cl_del_cancel_{_cl_id}"):
                            st.session_state.pop(f"confirm_del_cli_{_cl_id}", None)
                            st.rerun()
                    st.markdown("</div>", unsafe_allow_html=True)

                # ── Formulário de edição ──
                if st.session_state.get(f"editando_cli_{_cl_id}", False):
                    st.markdown(
                        "<div style='background:#0d1a0d;border:1.5px solid #ff2b2b44;border-radius:12px;"
                        "padding:16px 18px;margin:6px 0 12px'>",
                        unsafe_allow_html=True)
                    st.markdown(
                        f"<div style='color:#ff2b2b;font-size:10px;font-weight:700;letter-spacing:2px;"
                        f"text-transform:uppercase;margin-bottom:10px'>✏️ Editando: {_cl_nome_d}</div>",
                        unsafe_allow_html=True)
                    _ee1, _ee2 = st.columns(2)
                    with _ee1:
                        _ed_cl_nome = st.text_input("Nome", value=_cl_nome_d, key=f"ed_cl_nome_{_cl_id}")
                    with _ee2:
                        _ed_cl_email = st.text_input("E-mail", value=str(_cl.get("email","") or ""), key=f"ed_cl_email_{_cl_id}")
                    _ee3, _ee4 = st.columns(2)
                    with _ee3:
                        _ed_cl_user = st.text_input("Login", value=_cl_user_d, key=f"ed_cl_user_{_cl_id}")
                    with _ee4:
                        _ed_cl_tel = st.text_input("📱 WhatsApp", value=_cl_tel_d, key=f"ed_cl_tel_{_cl_id}")
                    _all_mercs_ed = db_read("SELECT mercado FROM mercados ORDER BY mercado")
                    _opts_mercs_ed = _all_mercs_ed["mercado"].tolist() if not _all_mercs_ed.empty else []
                    _mercs_atuais = [m.strip() for m in _cl_mercs_d.split(",") if m.strip() and m != "Todos os mercados"]
                    _ed_cl_mercs = st.multiselect("Mercados visíveis", _opts_mercs_ed,
                                                   default=[m for m in _mercs_atuais if m in _opts_mercs_ed],
                                                   key=f"ed_cl_mercs_{_cl_id}")
                    _ed_cl_ativo = st.checkbox("Conta ativa", value=bool(_cl_ativo_d), key=f"ed_cl_ativo_{_cl_id}")
                    _esb1, _esb2 = st.columns(2)
                    with _esb1:
                        if st.button("💾 Salvar alterações", key=f"ed_cl_save_{_cl_id}"):
                            db_exec("UPDATE clientes SET nome=?,email=?,usuario=?,telefone=?,mercados=?,ativo=? WHERE id=?",
                                    (_ed_cl_nome.strip(), _ed_cl_email.strip(), _ed_cl_user.strip(),
                                     _ed_cl_tel.strip(), ",".join(_ed_cl_mercs), int(_ed_cl_ativo), _cl_id))
                            st.session_state[f"editando_cli_{_cl_id}"] = False
                            st.success("✅ Cliente atualizado!")
                            st.rerun()
                    with _esb2:
                        if st.button("❌ Cancelar", key=f"ed_cl_cancel_{_cl_id}"):
                            st.session_state[f"editando_cli_{_cl_id}"] = False
                            st.rerun()
                    st.markdown("</div>", unsafe_allow_html=True)
        ...
        # ── PROMOTORES ATIVOS POR MERCADO ──
        st.markdown("<div style='height:24px'></div>", unsafe_allow_html=True)
        section_title("📊 Promotores ativos por mercado")

        _hj_adm = today_br()
        _dia_semana_map = {0:"Segunda",1:"Terça",2:"Quarta",3:"Quinta",4:"Sexta",5:"Sábado",6:"Domingo"}
        _dia_hoje_str = _dia_semana_map.get(_hj_adm.weekday(), "Segunda")

        _rel_hj_cl = db_read(
            "SELECT * FROM relatorio WHERE data=? AND funcionario NOT IN "
            "(SELECT usuario FROM usuarios WHERE tipo='teste')",
            (str(_hj_adm),))
        _agenda_hj = db_read(
            "SELECT funcionario, mercado, produto FROM agenda WHERE dia=?", (_dia_hoje_str,))

        _todos_mercs = db_read("SELECT mercado FROM mercados ORDER BY mercado")
        if not _todos_mercs.empty:
            for _, _mr in _todos_mercs.iterrows():
                _merc_nm = str(_mr["mercado"])
                _ag_merc = _agenda_hj[_agenda_hj["mercado"] == _merc_nm] if not _agenda_hj.empty else pd.DataFrame()
                _rel_merc = _rel_hj_cl[_rel_hj_cl["mercado"] == _merc_nm] if not _rel_hj_cl.empty else pd.DataFrame()

                _promo_agenda = _ag_merc["funcionario"].unique().tolist() if not _ag_merc.empty else []
                _promo_rel    = _rel_merc["funcionario"].unique().tolist() if not _rel_merc.empty else []
                _n_ativos     = len(_promo_rel)
                _n_agenda     = len(_promo_agenda)

                if _n_agenda == 0:
                    continue  # mercado sem agenda hoje, pula

                # Horas de abastecimento
                _horas_info = []
                if not _rel_merc.empty:
                    for _pf in _promo_rel:
                        _ci_p = db_read(
                            "SELECT hora_entrada, hora_saida FROM checkin WHERE funcionario=? AND data=? AND mercado=?",
                            (_pf, str(_hj_adm), _merc_nm))
                        if not _ci_p.empty and _ci_p.iloc[0].get("hora_entrada") and _ci_p.iloc[0].get("hora_saida"):
                            try:
                                _ta = datetime.strptime(str(_ci_p.iloc[0]["hora_entrada"]).strip(), "%H:%M")
                                _tb = datetime.strptime(str(_ci_p.iloc[0]["hora_saida"]).strip(), "%H:%M")
                                _mins = int((_tb - _ta).total_seconds() / 60)
                                if _mins > 0:
                                    _horas_info.append((_pf, _mins))
                            except: pass

                _cor_card = "#0a2b14" if _n_ativos > 0 else "#111"
                _borda    = "#22c55e44" if _n_ativos > 0 else "#1c1c1c"
                _produtos_dia = _ag_merc["produto"].unique().tolist() if not _ag_merc.empty else []
                _prods_str = ", ".join(_produtos_dia[:4]) + ("..." if len(_produtos_dia) > 4 else "")

                _horas_str = ""
                _total_min = sum(h[1] for h in _horas_info)
                if _total_min > 0:
                    _horas_str = f"⏱️ {_total_min//60}h{_total_min%60:02d}min de abastecimento hoje"

                # Comparativo horas semana
                _horas_semana = {}
                _dias_lista = ["Segunda","Terça","Quarta","Quinta","Sexta","Sábado","Domingo"]
                for _ds in _dias_lista:
                    _data_ds = _hj_adm - timedelta(days=(_hj_adm.weekday() - _dias_lista.index(_ds)) % 7)
                    _rel_ds = db_read("SELECT funcionario FROM relatorio WHERE data=? AND mercado=?",
                                      (str(_data_ds), _merc_nm))
                    if not _rel_ds.empty:
                        _ci_ds = db_read(
                            "SELECT hora_entrada, hora_saida FROM checkin WHERE data=? AND mercado=?",
                            (str(_data_ds), _merc_nm))
                        _min_ds = 0
                        for _, _cir in _ci_ds.iterrows():
                            try:
                                _ta2 = datetime.strptime(str(_cir["hora_entrada"]).strip(), "%H:%M")
                                _tb2 = datetime.strptime(str(_cir["hora_saida"]).strip(), "%H:%M")
                                _m2 = int((_tb2 - _ta2).total_seconds()/60)
                                if _m2 > 0: _min_ds += _m2
                            except: pass
                        if _min_ds > 0:
                            _horas_semana[_ds] = _min_ds

                with st.expander(f"🏪 {_merc_nm} — {_n_ativos}/{_n_agenda} promotores ativos"):
                    _pm1, _pm2, _pm3 = st.columns(3)
                    _pm1.metric("Promotores na agenda", _n_agenda)
                    _pm2.metric("Ativos hoje", _n_ativos)
                    _pm3.metric("Produtos no dia", len(_produtos_dia))

                    if _prods_str:
                        st.markdown(
                            f"<div style='background:#111;border:1px solid #1c1c1c;border-radius:8px;"
                            f"padding:8px 14px;margin:6px 0;color:#888;font-size:12px'>"
                            f"📦 <b style='color:#ddd'>Produtos abastecendo:</b> {_prods_str}</div>",
                            unsafe_allow_html=True)

                    if _horas_str:
                        st.markdown(
                            f"<div style='background:#0a2b14;border:1px solid #22c55e33;border-radius:8px;"
                            f"padding:8px 14px;margin:6px 0;color:#22c55e;font-size:12px;font-weight:700'>"
                            f"{_horas_str}</div>",
                            unsafe_allow_html=True)

                    if _promo_rel:
                        st.markdown("<div style='color:#555;font-size:10px;text-transform:uppercase;letter-spacing:1px;margin:8px 0 4px'>Promotores ativos</div>", unsafe_allow_html=True)
                        for _pf2 in _promo_rel:
                            _hm = next((h[1] for h in _horas_info if h[0] == _pf2), None)
                            _hm_str = f" · ⏱️ {_hm//60}h{_hm%60:02d}min" if _hm else ""
                            st.markdown(
                                f"<div style='color:#ddd;font-size:12px;padding:3px 0'>"
                                f"✅ {_pf2.split('.')[0].capitalize()}{_hm_str}</div>",
                                unsafe_allow_html=True)

                    if _horas_semana:
                        st.markdown("<div style='color:#555;font-size:10px;text-transform:uppercase;letter-spacing:1px;margin:10px 0 4px'>Horas de abastecimento — semana</div>", unsafe_allow_html=True)
                        _max_min = max(_horas_semana.values())
                        for _ds2, _min_ds2 in _horas_semana.items():
                            _pct_ds = int(_min_ds2 / _max_min * 100) if _max_min else 0
                            _hs2 = f"{_min_ds2//60}h{_min_ds2%60:02d}min"
                            _cor_ds = "#ff2b2b" if _ds2 == _dia_hoje_str else "#3b82f6"
                            st.markdown(
                                f"<div style='margin-bottom:5px'>"
                                f"<div style='display:flex;justify-content:space-between;margin-bottom:2px'>"
                                f"<span style='color:#888;font-size:11px'>{_ds2}</span>"
                                f"<span style='color:{_cor_ds};font-size:11px;font-weight:700'>{_hs2}</span></div>"
                                f"<div style='background:#1a1a1a;border-radius:4px;height:5px'>"
                                f"<div style='background:{_cor_ds};width:{_pct_ds}%;height:5px;border-radius:4px'></div></div></div>",
                                unsafe_allow_html=True)
...
elif tipo_efetivo == "cliente":
    _cli_row = db_read("SELECT * FROM clientes WHERE LOWER(usuario)=LOWER(?)", (usuario,))
    if _cli_row.empty:
        st.error("Conta de cliente não encontrada.")
        st.stop()
    _cli = _cli_row.iloc[0]
    _cli_nome_disp = _cli.get("nome", usuario)
    _cli_mercs_str = _cli.get("mercados","") or ""
    _cli_mercs = [m.strip() for m in _cli_mercs_str.split(",") if m.strip()]

    # Sidebar do cliente
    with st.sidebar:
        st.markdown(
            f"<div style='color:#555;font-size:10px;font-weight:700;letter-spacing:1.5px;"
            f"text-transform:uppercase;padding:8px 0 2px'>LOGADO COMO</div>"
            f"<div style='color:#ddd;font-size:14px;font-weight:700'>🏢 {_cli_nome_disp}</div>",
            unsafe_allow_html=True)
        st.markdown("<hr style='border-color:#1c1c1c;margin:8px 0'>", unsafe_allow_html=True)
        if st.button("🚪 Sair", key="sair_cliente", use_container_width=True):
            for _k in list(st.session_state.keys()): del st.session_state[_k]
            st.rerun()

    # Header
    st.markdown(
        f"<div style='margin-bottom:24px'>"
        f"<div style='color:#ff2b2b;font-size:10px;font-weight:700;letter-spacing:2px'>EL KAM MERCHANDISING</div>"
        f"<div style='color:#fff;font-size:26px;font-weight:900;margin:4px 0'>Olá, {_cli_nome_disp} 👋</div>"
        f"<div style='color:#444;font-size:13px'>Acompanhe a execução no ponto de venda</div></div>",
        unsafe_allow_html=True)

    # Filtro de período
    _cli_per = st.selectbox("Período", ["Hoje","Últimos 7 dias","Últimos 30 dias"], key="cl_per")
    _cl_hj = today_br()
    _cl_d1 = _cl_hj if _cli_per == "Hoje" else (_cl_hj - timedelta(days=7) if "7" in _cli_per else _cl_hj - timedelta(days=30))
    _cl_d2 = _cl_hj

    # Query filtrada pelos mercados do cliente
    _no_teste = "AND funcionario NOT IN (SELECT usuario FROM usuarios WHERE tipo='teste')"
    if _cli_mercs:
        _ph = ",".join(["?"]*len(_cli_mercs))
        _cl_rel = db_read(
            f"SELECT * FROM relatorio WHERE data>=? AND data<=? AND mercado IN ({_ph}) {_no_teste}",
            (_cl_d1.isoformat(), _cl_d2.isoformat(), *_cli_mercs))
    else:
        _cl_rel = db_read(
            f"SELECT * FROM relatorio WHERE data>=? AND data<=? {_no_teste}",
            (_cl_d1.isoformat(), _cl_d2.isoformat()))

    # Métricas
    _cm1,_cm2,_cm3,_cm4 = st.columns(4)
    _cl_vis   = len(_cl_rel) if not _cl_rel.empty else 0
    _cl_mv    = _cl_rel["mercado"].nunique() if not _cl_rel.empty else 0
    _cl_flt   = len(_cl_rel[_cl_rel["status"]=="falta"]) if not _cl_rel.empty else 0
    _cl_taxa  = int((_cl_vis-_cl_flt)/_cl_vis*100) if _cl_vis > 0 else 0
    _cm1.metric("📊 Visitas realizadas", _cl_vis)
    _cm2.metric("🏪 Mercados visitados", _cl_mv)
    _cm3.metric("⚠️ Produtos em falta", _cl_flt)
    _cm4.metric("📦 Taxa abastecimento", f"{_cl_taxa}%")

    st.markdown("<hr style='border-color:#1c1c1c;margin:20px 0'>", unsafe_allow_html=True)

    # Tabela de relatórios
    if not _cl_rel.empty:
        ...
        _badge_cl = {"abastecido":"✅ Abastecido","falta":"⚠️ Em falta","fechado":"❌ Fechada"}
        _cor_cl   = {"abastecido":"#001a08","falta":"#1a1000","fechado":"#1a0000"}
        for _, _cr in _cl_rel.sort_values("data", ascending=False).iterrows():
            _st2 = str(_cr.get("status","")).lower().strip()
            st.markdown(
                f"<div style='background:{_cor_cl.get(_st2,'#111')};border:1px solid #222;"
                f"border-radius:8px;padding:10px 16px;margin:3px 0;"
                f"display:flex;justify-content:space-between;align-items:center'>"
                f"<div>"
                f"<div style='color:#fff;font-size:13px;font-weight:700'>"
                f"🏪 {_cr['mercado']}  ·  📦 {_cr['produto']}</div>"
                f"<div style='color:#555;font-size:11px;margin-top:2px'>"
                f"👤 {_cr['funcionario'].split('.')[0].capitalize()}  ·  "
                f"📅 {_cl_fmt_d(_cr['data'])}"
                f"{'  ·  🕐 '+str(_cr.get('hora','')) if str(_cr.get('hora','')) else ''}</div></div>"
                f"<span style='font-size:12px;color:#aaa'>{_badge_cl.get(_st2,_st2)}</span></div>",
                unsafe_allow_html=True)

        # Download PDF
        st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
        _pdf_rows = "".join(
            f"<tr><td>{_cl_fmt_d(r['data'])}</td><td>{r['funcionario'].split('.')[0].capitalize()}</td>"
            f"<td>{r['mercado']}</td><td>{r['produto']}</td>"
            f"<td>{'✅ Abastecido' if r.get('status')=='abastecido' else ('⚠️ Em falta' if r.get('status')=='falta' else '❌ Fechada')}</td>"
            f"<td>{r.get('hora','') or ''}</td></tr>"
            for _, r in _cl_rel.sort_values("data", ascending=False).iterrows()
        )
        _pdf_html = f"""<!DOCTYPE html><html><head><meta charset='utf-8'>
        <style>body{{font-family:Arial,sans-serif;padding:20px;color:#222}}
        h2{{color:#ff2b2b;border-bottom:3px solid #ff2b2b;padding-bottom:8px}}
        table{{width:100%;border-collapse:collapse;margin-top:16px}}
        th{{background:#ff2b2b;color:#fff;padding:8px 10px;font-size:12px;text-align:left}}
        td{{padding:7px 10px;border-bottom:1px solid #eee;font-size:12px}}
        tr:nth-child(even){{background:#f9f9f9}}
        .footer{{color:#bbb;font-size:10px;text-align:center;margin-top:20px}}
        </style></head><body>
        <h2>El Kam Merchandising — Relatório</h2>
        <p style='color:#888;font-size:12px'>Cliente: {_cli_nome_disp} · Período: {_cl_fmt_d(_cl_d1)} a {_cl_fmt_d(_cl_d2)}</p>
        <table><thead><tr><th>Data</th><th>Promotor</th><th>Mercado</th><th>Produto</th><th>Status</th><th>Hora</th></tr></thead>
        <tbody>{_pdf_rows}</tbody></table>
        <div class='footer'>© El Kam Merchandising — Todos os direitos reservados.</div>
        </body></html>"""
        st.download_button("📄 Baixar relatório PDF/HTML",
                           data=_pdf_html.encode("utf-8"),
                           file_name=f"relatorio_elkam_{_cl_d1}.html",
                           mime="text/html",
                           use_container_width=True)
    else:
        st.info("Nenhum dado encontrado para o período selecionado.")

    # ── COMPARATIVO DE PREÇOS — até 30 dias ──
    st.markdown("<div style='height:24px'></div>", unsafe_allow_html=True)
    section_title("🏷️ Comparativo de Preços — até 30 dias")
    st.caption("Acompanhe a evolução dos preços por mercado e produto.")

    _cp_d1 = _cl_hj - timedelta(days=30)
    _no_teste_cp = "AND funcionario NOT IN (SELECT usuario FROM usuarios WHERE tipo='teste')"
    if _cli_mercs:
        _ph_cp = ",".join(["?"]*len(_cli_mercs))
        _cp_rel = db_read(
            f"SELECT data, mercado, produto, preco_valor, foto_preco, foto, hora, funcionario "
            f"FROM relatorio WHERE data>=? AND data<=? AND mercado IN ({_ph_cp}) "
            f"AND (preco_valor!='' OR foto_preco!='') {_no_teste_cp} ORDER BY data DESC",
            (_cp_d1.isoformat(), _cl_hj.isoformat(), *_cli_mercs))
    else:
        _cp_rel = db_read(
            f"SELECT data, mercado, produto, preco_valor, foto_preco, foto, hora, funcionario "
            f"FROM relatorio WHERE data>=? AND data<=? "
            f"AND (preco_valor!='' OR foto_preco!='') {_no_teste_cp} ORDER BY data DESC",
            (_cp_d1.isoformat(), _cl_hj.isoformat()))

    if _cp_rel.empty:
        st.info("Nenhum preço registrado nos últimos 30 dias.")
    else:
        # Filtros
        _fp_c1, _fp_c2 = st.columns(2)
        with _fp_c1:
            _mercs_cp = ["Todos"] + sorted(_cp_rel["mercado"].unique().tolist())
            _filt_merc_cp = st.selectbox("🏪 Filtrar por mercado", _mercs_cp, key="cp_merc_filt")
        with _fp_c2:
            _prods_cp = ["Todos"] + sorted(_cp_rel["produto"].unique().tolist())
            _filt_prod_cp = st.selectbox("📦 Filtrar por produto", _prods_cp, key="cp_prod_filt")

        _cp_filt = _cp_rel.copy()
        if _filt_merc_cp != "Todos":
            _cp_filt = _cp_filt[_cp_filt["mercado"] == _filt_merc_cp]
        if _filt_prod_cp != "Todos":
            _cp_filt = _cp_filt[_cp_filt["produto"] == _filt_prod_cp]

        if _cp_filt.empty:
            st.info("Nenhum preço registrado com esses filtros.")
        else:
            for _, _cp_row in _cp_filt.iterrows():
                _pv = str(_cp_row.get("preco_valor","") or "")
                _fp = str(_cp_row.get("foto_preco","") or "")
                _fd = str(_cp_row.get("foto","") or "")
                _cp_d = _cp_row.get("data","")
                _cp_m = str(_cp_row.get("mercado",""))
                _cp_p = str(_cp_row.get("produto",""))
                _cp_h = str(_cp_row.get("hora","") or "")
                _cp_f = str(_cp_row.get("funcionario","") or "").split(".")[0].capitalize()

                # Comparativo com dia anterior
                _ontem_cp = (pd.to_datetime(_cp_d) - timedelta(days=1)).strftime("%Y-%m-%d") if _cp_d else ""
                _preco_ontem = ""
                if _ontem_cp:
                    _row_ontem = db_read(
                        "SELECT preco_valor FROM relatorio WHERE data=? AND mercado=? AND produto=? AND preco_valor!=''",
                        (_ontem_cp, _cp_m, _cp_p))
                    if not _row_ontem.empty:
                        _preco_ontem = str(_row_ontem.iloc[0]["preco_valor"])

                _delta_html = ""
                if _pv and _preco_ontem:
                    try:
                        _pv_f = float(_pv.replace(",",".").replace("R$","").strip())
                        _po_f = float(_preco_ontem.replace(",",".").replace("R$","").strip())
                        _diff = _pv_f - _po_f
                        if _diff > 0:
                            _delta_html = f"<span style='color:#ff4444;font-size:11px'>▲ R$ {_diff:.2f} vs ontem</span>"
                        elif _diff < 0:
                            _delta_html = f"<span style='color:#22c55e;font-size:11px'>▼ R$ {abs(_diff):.2f} vs ontem</span>"
                        else:
                            _delta_html = "<span style='color:#888;font-size:11px'>= Mesmo preço que ontem</span>"
                    except: pass

                _cp_cols = st.columns([3, 2])
                with _cp_cols[0]:
                    st.markdown(
                        f"<div style='background:#111;border:1px solid #1c1c1c;border-radius:10px;"
                        f"padding:10px 14px;margin:3px 0'>"
                        f"<div style='color:#fff;font-size:13px;font-weight:700'>🏪 {_cp_m}  ·  📦 {_cp_p}</div>"
                        f"<div style='color:#555;font-size:11px;margin-top:2px'>"
                        f"📅 {_cl_fmt_d(_cp_d)}{'  🕐 '+_cp_h if _cp_h else ''}  ·  👤 {_cp_f}</div>"
                        f"{'<div style=margin-top:6px><span style=background:#1a2b00;border:1px solid #3b82f644;border-radius:6px;padding:3px 10px;color:#3b82f6;font-size:13px;font-weight:700>💰 R$ '+_pv+'</span></div>' if _pv else ''}"
                        f"{'<div style=margin-top:4px>'+_delta_html+'</div>' if _delta_html else ''}"
                        f"</div>",
                        unsafe_allow_html=True)
                with _cp_cols[1]:
                    _fotos_mostradas = 0
                    if _fp and os.path.exists(_fp):
                        st.image(_fp, use_container_width=True, caption="🏷️ Foto do preço")
                        _fotos_mostradas += 1
                    if _fd and os.path.exists(_fd) and _fotos_mostradas == 0:
                        st.image(_fd, use_container_width=True, caption="📸 Gôndola")
...
# ═══════════════════════════════════════════════════════
    ...
    # Sidebar: botões de navegação (desktop) — Sair já está no topo via sair_global
    with st.sidebar:
        st.markdown("<hr style='border-color:#1c1c1c;margin:4px 0 10px'>", unsafe_allow_html=True)
    ...
    # ── AGENDA ──
        ...
        # Mercados já com relatório enviado hoje são ocultados (some após envio)
        _mercs_com_rel = set()
        if _dia_e_hoje:
            _rels_hoje = db_read(
                "SELECT DISTINCT mercado FROM relatorio WHERE funcionario=? AND data=?",
                (usuario, str(today_br())))
            if not _rels_hoje.empty:
                _mercs_com_rel = set(_rels_hoje["mercado"].tolist())

        _mercs_restantes = [m for m in dados_dia["mercado"].unique() if m not in _mercs_com_rel]

        if _dia_e_hoje and _mercs_com_rel:
            _total_m = len(dados_dia["mercado"].unique())
            _done_m  = len(_mercs_com_rel)
            _pct_m   = int(_done_m / _total_m * 100)
            st.markdown(
                f"<div style='background:#0a2b14;border:1px solid #22c55e33;border-radius:12px;"
                f"padding:12px 18px;margin:0 0 16px'>"
                f"<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:6px'>"
                f"<span style='color:#22c55e;font-size:13px;font-weight:700'>✅ Progresso do dia</span>"
                f"<span style='color:#22c55e;font-size:13px;font-weight:700'>{_done_m}/{_total_m} mercados</span></div>"
                f"<div style='background:#0d3a1a;border-radius:4px;height:6px'>"
                f"<div style='background:#22c55e;width:{_pct_m}%;height:6px;border-radius:4px'></div></div></div>",
                unsafe_allow_html=True)

        if not _mercs_restantes:
            st.markdown(
                "<div style='background:linear-gradient(135deg,#0a2b14,#0d3a1a);"
                "border:1px solid #22c55e44;border-radius:16px;padding:32px;text-align:center;margin-top:12px'>"
                "<div style='font-size:48px;margin-bottom:12px'>🏆</div>"
                "<div style='color:#22c55e;font-size:20px;font-weight:900;margin-bottom:8px'>"
                "Todas as visitas concluídas!</div>"
                "<div style='color:#555;font-size:14px'>Parabéns pelo excelente trabalho hoje! 💪</div>"
                "</div>", unsafe_allow_html=True)

        for merc in _mercs_restantes:
            ...
            for i, r in prods.iterrows():
                ...
                # Câmera — só mostra se produto NÃO está abastecido
                chave_mostrar_cam = f"mostrar_cam_{merc}_{dia_sel}_{i}"
                if abast_prod:
                    # Produto já marcado como abastecido — esconde câmera
                    st.session_state[chave_mostrar_cam] = False
                elif not st.session_state.get(chave_mostrar_cam, False):
                    if _dia_e_hoje:
                ...
                # ── FOTO DO PREÇO ──
                st.markdown(
                    "<div style='background:#0a0a1a;border:1px solid #3b82f622;border-radius:10px;"
                    "padding:10px 14px;margin:8px 0 4px'>"
                    "<div style='color:#3b82f6;font-size:11px;font-weight:700;margin-bottom:6px'>"
                    "🏷️ Foto do preço (opcional)</div>",
                    unsafe_allow_html=True)
                _fp_preco_path = f"fotos/{usuario}/{merc}_{dia_sel}_{prod_nome}_{today_br()}_preco.jpg".replace(" ","_")
                _tem_foto_preco = os.path.exists(_fp_preco_path)
                _chave_cam_preco = f"mostrar_cam_preco_{merc}_{dia_sel}_{i}"
                _chave_preco_val = f"preco_val_{merc}_{dia_sel}_{i}"

                _pv1, _pv2 = st.columns([2,1])
                with _pv1:
                    _preco_digitado = st.text_input(
                        "💰 Valor do preço (R$)", key=_chave_preco_val,
                        placeholder="Ex: 3,49",
                        label_visibility="collapsed" if not _dia_e_hoje else "visible")
                with _pv2:
                    if _tem_foto_preco:
                        st.image(_fp_preco_path, use_container_width=True, caption="🏷️ Foto do preço")
                        if st.button("🗑️", key=f"del_preco_{merc}_{dia_sel}_{i}", help="Apagar foto do preço"):
                            os.remove(_fp_preco_path)
                            st.rerun()
                    elif _dia_e_hoje:
                        if not st.session_state.get(_chave_cam_preco, False):
                            if st.button("📸 Foto do preço", key=f"abrir_cam_preco_{merc}_{dia_sel}_{i}",
                                         use_container_width=True):
                                st.session_state[_chave_cam_preco] = True
                                st.rerun()
                        else:
                            if st.button("✖ Fechar", key=f"fechar_cam_preco_{merc}_{dia_sel}_{i}"):
                                st.session_state[_chave_cam_preco] = False
                                st.rerun()
                            fc_preco = st.camera_input("Foto do preço", key=f"cam_preco_{merc}_{dia_sel}_{i}",
                                                       label_visibility="collapsed")
                            if fc_preco:
                                from PIL import Image, ImageDraw, ImageFont
                                import io
                                _img_preco = Image.open(fc_preco)
                                _draw_preco = ImageDraw.Draw(_img_preco)
                                _ts_preco = now_br().strftime("%d/%m/%Y %H:%M:%S")
                                _txt_preco = f"El Kam · Preço · {usuario} · {_ts_preco}"
                                _w_pr, _h_pr = _img_preco.size
                                _fsz_pr = max(14, _w_pr // 30)
                                try:
                                    _fnt_pr = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", _fsz_pr)
                                except:
                                    _fnt_pr = ImageFont.load_default()
                                _bb_pr = _draw_preco.textbbox((0,0), _txt_preco, font=_fnt_pr)
                                _tw_pr = _bb_pr[2]-_bb_pr[0]; _th_pr = _bb_pr[3]-_bb_pr[1]
                                _pad_pr = 6; _x_pr = _pad_pr; _y_pr = _h_pr - _th_pr - _pad_pr*3
                                _draw_preco.rectangle([_x_pr-_pad_pr, _y_pr-_pad_pr,
                                                       _x_pr+_tw_pr+_pad_pr, _y_pr+_th_pr+_pad_pr],
                                                      fill=(0,0,0,160))
                                _draw_preco.text((_x_pr, _y_pr), _txt_preco,
                                                 fill=(255,255,255,255), font=_fnt_pr)
                                _buf_pr = io.BytesIO()
                                _img_preco.save(_buf_pr, format="JPEG", quality=88)
                                with open(_fp_preco_path,"wb") as _fp_pr: _fp_pr.write(_buf_pr.getvalue())
                                st.session_state[_chave_cam_preco] = False
                                st.rerun()
                st.markdown("</div>", unsafe_allow_html=True)
            ...
            # Confirmação: foto obrigatória APENAS para abastecido
            # Loja fechada e produto em falta → pode enviar sem foto (documenta a ocorrência)
            _status_atual = {0:"falta", 1:"fechado", 2:"abastecido"}.get(_st_idx, "abastecido")
            if _status_atual == "abastecido":
                tem_qualquer_foto = bool(caminho)
            else:
                tem_qualquer_foto = True  # fechado/falta não precisa de foto para liberar envio

            if _status_atual == "abastecido" and not caminho:
                st.markdown(
                    ...
                    "📷 Tire a foto de pelo menos um produto para confirmar o abastecimento</div>",
                    unsafe_allow_html=True)
            ...
            # ── ENVIAR RELATÓRIO ──
            # (mercados já enviados não aparecem nesta lista)
            if st.button("✅ Enviar relatório", key=f"btn_{merc}_{dia_sel}",
                ...
                for _, _rp2 in prods.iterrows():
                    ...
                    # ── FOTO OBRIGATÓRIA só para abastecido ──
                    if not os.path.exists(_fp2):
                        if status_val == "abastecido":
                            st.warning(f"⚠️ Produto '{_prod_nm}' sem foto — ignorado.")
                            continue
                        else:
                            # Fechado ou falta: salva sem foto
                            _fp2 = ""
                            _fb64_prod = ""
                            _foto_hash = ""
                    ...
                    try:
                        _gps_lat_v = float(st.session_state.get(f"gps_lat_{merc}", 0) or 0)
                        _gps_lon_v = float(st.session_state.get(f"gps_lon_{merc}", 0) or 0)
                    except:
                        _gps_lat_v = None
                        _gps_lon_v = None
                    
                    # ── SALVA RELATÓRIO ──
                    _fp_preco_salvo = f"fotos/{usuario}/{merc}_{dia_sel}_{_prod_nm}_{today_br()}_preco.jpg".replace(" ","_")
                    _foto_preco_path = _fp_preco_salvo if os.path.exists(_fp_preco_salvo) else ""
                    # Busca valor do preço pelo nome do produto (chave usa índice do df, varre session_state)
                    _preco_val_salvo = ""
                    for _sk, _sv in st.session_state.items():
                        if _sk.startswith(f"preco_val_{merc}_{dia_sel}_") and _sv:
                            # verifica se é a chave deste produto verificando índice no df
                            try:
                                _idx_str = _sk.split(f"preco_val_{merc}_{dia_sel}_")[1]
                                _idx_int = int(_idx_str)
                                if prods.loc[_idx_int, "produto"] == _prod_nm:
                                    _preco_val_salvo = str(_sv)
                                    break
                            except: pass
                    db_exec(
                        "INSERT INTO relatorio "
                        "(data,funcionario,mercado,produto,status,foto,"
                        "produto_faltante,foto_b64,hora,enviado_mercado,notif_admin,lat,lon,foto_hash,foto_preco,preco_valor) "
                        "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        (_data_hoje, usuario, merc, _prod_nm, _st_prod, _fp2,
                         _pf_prod, _fb64_prod, _hora_agora, 0, 1,
                         _gps_lat_v, _gps_lon_v, _foto_hash, _foto_preco_path, str(_preco_val_salvo)))
                ...
                # ── Fotos ficam em disco (func pode navegar e voltar sem perder estado) ──
                # Apenas limpa chaves de câmera abertas, mantém abast_* intactos
                keys_del = [k for k in list(st.session_state.keys())
                            if k.startswith("mostrar_cam_") and merc.replace(" ","_") in k]
                for k in keys_del:
                    del st.session_state[k]

                # ── Notificação ao admin (ou superadmin para conta teste) ──
                ...
                _msg_wa = (
                    ...
                    f"🏪 {merc}\n"
                    f"📅 {today_br().strftime('%d/%m/%Y')}  🕐 {_hora_co}\n"
                    f"Status: {_badge_wa}{_pf_wa}\n"
                    f"\n📦 *Produtos:*\n{_prods_wa_str}"
                )
                # Conta teste → telefone do superadmin; demais → telefone do admin
                _is_conta_teste = (usuario == "teste.teste")
                if _is_conta_teste:
                    _dest_row = db_read("SELECT telefone FROM usuarios WHERE tipo='superadmin'")
                else:
                    _dest_row = db_read("SELECT telefone FROM usuarios WHERE tipo='admin'")
                _dest_tel = _dest_row.iloc[0]["telefone"] if not _dest_row.empty else ""
                if _dest_tel:
                    _wa_m = f"https://wa.me/55{_dest_tel}?text={_up.quote(_msg_wa)}"
                    _wa_p = f"https://web.whatsapp.com/send?phone=55{_dest_tel}&text={_up.quote(_msg_wa)}"
                    _dest_label = "superadmin" if _is_conta_teste else "admin"
                    st.markdown(
                        ...
                        f"✅ Relatório enviado! Toque abaixo para avisar o {_dest_label} pelo WhatsApp</div>"
                ...
                st.rerun()  # Força atualização imediata após salvar relatório
