...
# ═══════════════════════════════════════════════════════
    ...
    # ── AGENDA ──
        ...
        </style><table class="tab-ag"><thead><tr><th>Mercado</th>"""
        ...
        for merc in dados_dia["mercado"].unique():
            ...
            # ── CARD DO MERCADO (sem expander) ──
            st.markdown(f"""
            <div style='background:#111;border:1px solid #1c1c1c;border-radius:14px;
                        padding:0;margin-bottom:24px;overflow:hidden'>
                <div style='background:#141414;border-bottom:1px solid #1c1c1c;
                            padding:14px 18px;display:flex;align-items:center;gap:14px'>
                    <div style='font-size:28px'>{"" if logo_merc and os.path.exists(logo_merc) else "🏪"}</div>
                    <div>
                        <div style='color:#ff2b2b;font-size:10px;font-weight:700;
                                    letter-spacing:2px;text-transform:uppercase'>Mercado</div>
                        <div style='color:#fff;font-size:18px;font-weight:800'>{merc}</div>
                        {"<div style='color:#444;font-size:11px;margin-top:2px'>📍 "+end_merc+"</div>" if end_merc else ""}
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)

            # Logo se existir — logo após o card header
            if logo_merc and os.path.exists(logo_merc):
                lc1, lc2 = st.columns([1, 6])
                with lc1:
                    st.image(logo_merc, width=60)
                with lc2:
                    if end_merc:
                        st.markdown(
                            f"<div style='color:#444;font-size:12px;padding-top:18px'>📍 {end_merc}</div>",
                            unsafe_allow_html=True)

            if end_merc:
                st.markdown(
                    f"[🗺️ Abrir no Google Maps](https://www.google.com/maps/search/{end_merc.replace(' ','+')})")
            ...
            st.markdown(
                ...
                "text-transform:uppercase;letter-spacing:1px;margin:12px 0 8px'>Produtos</div>",
                unsafe_allow_html=True)
            ...
            for i, r in prods.iterrows():
                ...
                # ── LINHA do produto ──
                lc_nome, lc_ref, lc_foto, lc_acao = st.columns([4, 2, 3, 2])
                ...
                with lc_nome:
                    st.markdown(
                        ...
                        f"text-transform:uppercase;margin-bottom:4px'>Produto</div>"
                        f"<div style='color:#fff;font-size:14px;font-weight:700'>{prod_nome}</div>"
                        f"</div>",
                        unsafe_allow_html=True)

                with lc_ref:
                    if tem_ref:
                        st.image(str(foto_ref.iloc[0]["foto"]), use_container_width=True,
                                 caption="Referência")
                    else:
                        st.markdown(
                            "<div style='color:#252525;font-size:10px;text-align:center;"
                            "padding:16px 0'>Sem foto<br>de ref.</div>",
                            unsafe_allow_html=True)

                with lc_foto:
                    if tem_foto_prod:
                        st.image(nome_foto_prod, use_container_width=True, caption="Foto tirada")
                    else:
                        st.markdown(
                            "<div style='color:#333;font-size:11px;text-align:center;"
                            "padding:20px 0;border:1px dashed #222;border-radius:8px'>"
                            "📷<br>Tire a foto</div>",
                            unsafe_allow_html=True)
                    # Upload / câmera
                    modo_p = st.radio("", ["📸","📁"],
                ...
                with lc_acao:
                    if tem_foto_prod:
                        ...
                        if st.button("🗑️ Apagar foto", key=f"del_fp_{merc}_{dia_sel}_{i}"):
                        ...
                        st.markdown(
                            "<div style='color:#2a2a2a;font-size:11px;padding:20px 0;text-align:center'>"
                            "Tire a foto<br>para marcar<br>abastecido</div>",
                            unsafe_allow_html=True)
                        abast_prod = False
            ...
            # Pré-seleciona "Abastecido" se todos os produtos tiverem foto+check
            idx_default = 0 if todos_abastecidos and len(prods) > 0 else 0
            status_sel = st.radio(
            ...
            # caminho da foto geral do mercado (se houver alguma foto de produto)
            fotos_do_merc = [
            ...
            confirmado = st.checkbox(
                ...
                help="Tire ao menos uma foto para confirmar")

            if not tem_qualquer_foto:
                st.markdown(
                    "<div style='color:#333;font-size:11px;margin-bottom:8px'>"
                    "📷 Tire a foto de pelo menos um produto para confirmar</div>",
                    unsafe_allow_html=True)
                ...
                if st.button("✅ Enviar relatório", key=f"btn_{merc}_{dia_sel}",
                    ...
                    if status_val == "abastecido":
                        ...
                        <div style='background:linear-gradient(135deg,#0a2b14,#0d3a1a);
                                    ...
                                    padding:20px 24px;text-align:center;animation:fadeIn 0.4s ease'>
                            <div style='font-size:40px;margin-bottom:10px'>🏆</div>
                            <div style='color:#22c55e;font-size:16px;font-weight:800;margin-bottom:6px'>
                                Gôndola abastecida!</div>
                            <div style='color:#888;font-size:13px;line-height:1.6'>
                                Excelente trabalho, {nome_exib}! 💪<br>
            ...
            with col_pdf:
                if st.button("📄 Gerar PDF", key=f"pdf_{merc}_{dia_sel}",
                             disabled=not confirmado):
                    status_val = status_opcoes[status_sel]
                    badge = {"abastecido":"✅ Abastecido","falta":"⚠️ Produto em falta","fechado":"❌ Loja fechada"}.get(status_val,"")
                    fotos_html = "".join(
                        f'<div style="margin:8px 0"><b>{r["produto"]}</b><br>'
                        f'<img src="{fp}" style="width:200px;border-radius:6px"></div>'
                        for _, r in prods.iterrows()
                    ...
                    rel_html = f"""<!DOCTYPE html><html><head><meta charset='utf-8'>
                    <style>body{{font-family:Arial;background:#fff;padding:20px;color:#222}}
                    .header{{background:#ff2b2b;color:#fff;padding:16px;border-radius:8px;margin-bottom:16px}}
                    .row{{margin:8px 0;font-size:14px}}</style></head><body>
                    <div class='header'><b>El Kam Merchandising</b> — Relatório de Visita</div>
                    <div class='row'><b>Funcionário:</b> {usuario}</div>
                    <div class='row'><b>Mercado:</b> {merc}</div>
                    <div class='row'><b>Data:</b> {date.today().strftime('%d/%m/%Y')} ({dia_sel})</div>
                    <div class='row'><b>Status:</b> {badge}</div>
                    {pf_tag}{fotos_html}
                    <p style='color:#aaa;font-size:11px;margin-top:20px'>© El Kam Merchandising</p>
                    </body></html>"""
                    ...
                    with open(html_path,"rb") as hf:
                        st.download_button("⬇️ Baixar", data=hf,
