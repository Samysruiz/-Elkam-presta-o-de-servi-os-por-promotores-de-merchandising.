            ...
            if st.button("✅ Enviar relatório", key=f"btn_{merc}_{dia_sel}",
                ...
                for _, _rp2 in prods.iterrows():
                    ...
                    if not os.path.exists(_fp2):
                        st.error("⚠️ Foto obrigatória antes de enviar.")
                        continue
                    ...
                    _preco_val_salvo = st.session_state.get(f"preco_val_{merc}_{dia_sel}_{list(prods['produto'].values).index(_prod_nm) if _prod_nm in list(prods['produto'].values) else 0}", "")
                    db_exec(
