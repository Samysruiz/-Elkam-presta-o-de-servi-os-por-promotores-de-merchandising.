...
if tipo_efetivo == "admin":
    ...
    # ── TOP NAV RÁPIDA (mobile) ──
    _nc1,_nc2,_nc3,_nc4,_nc5 = st.columns(5)
    with _nc1:
        if st.button("📊\nDash", key="mn_d", use_container_width=True):
            st.session_state["adm_menu"] = "Dashboard"; st.rerun()
    with _nc2:
        if st.button("👥\nFuncs", key="mn_f", use_container_width=True):
            st.session_state["adm_menu"] = "Funcionários"; st.rerun()
    with _nc3:
        if st.button("📈\nRelat.", key="mn_r", use_container_width=True):
            st.session_state["adm_menu"] = "Relatórios"; st.rerun()
    with _nc4:
        if st.button("💬\nChat", key="mn_c", use_container_width=True):
            st.session_state["adm_menu"] = "Chat"; st.rerun()
    with _nc5:
        if st.button("⚙️\nConf.", key="mn_cfg", use_container_width=True):
            st.session_state["adm_menu"] = "⚙️ Configurações"; st.rerun()
