...
if tipo_efetivo == "admin":
    ...
    # ── RELATÓRIOS ────────────────────────────────────────
        ...
        def _gerar_html_rel(df_rows, titulo="El Kam Merchandising", subtitulo="", apagar_fotos=False):
            """Gera HTML profissional agrupado por mercado, com foto por produto."""
            if df_rows.empty:
            ...
            for _mh in sorted(_mercs_h):
                ...
                _data_mh  = str(_df_mh.iloc[0]["data"])[:10] if not _df_mh.empty else ""
                ...
                for _, _rh2 in _df_mh.iterrows():
                    ...
                    _cor_r   = {"abastecido":"#d4edda","falta":"#fff3cd","fechado":"#f8d7da"}.get(_st_r, "#fff")
                    _pf_r    = _rh2.get("produto_faltante","") or ""
                    _b64_r   = _rh2.get("foto_b64","") or ""
                    _foto_r  = f'<img src="data:image/jpeg;base64,{_b64_r}" style="max-width:200px;max-height:160px;border-radius:8px;border:2px solid #eee">' if _b64_r.strip() else "<span style='color:#aaa;font-style:italic'>Sem foto</span>"
                    _prods_rows += (
                        f"<tr style='background:{_cor_r}'>"
                        f"<td style='padding:10px 12px;font-weight:600'>{_rh2.get('produto','—')}</td>"
                        f"<td style='padding:10px 12px'>{_badge_r}</td>"
                        f"<td style='padding:10px 12px;color:#c00'>{_pf_r or '—'}</td>"
                        f"<td style='padding:10px 12px'>{_foto_r}</td>"
                        f"</tr>"
                    )
                    if apagar_fotos:
                ...
                _blocos += (
                    f"<div style='border:1.5px solid #ddd;border-radius:12px;margin-bottom:28px;overflow:hidden'>"
                    f"<div style='background:#ff2b2b;color:#fff;padding:12px 18px;display:flex;justify-content:space-between;align-items:center'>"
                    f"<span style='font-size:16px;font-weight:900'>🏪 {_mh}</span>"
                    f"<span style='font-size:12px'>📅 {_data_mh}  🕐 {_hora_mh}  ·  "
                    f"👤 {', '.join(f.split(chr(46))[0].capitalize() for f in _funcs_mh)}</span>"
                    f"</div>"
                    f"<table style='width:100%;border-collapse:collapse;font-size:13px'>"
                    f"<thead><tr style='background:#f5f5f5'>"
                    f"<th style='padding:9px 12px;text-align:left;border-bottom:2px solid #ddd'>Produto</th>"
                    f"<th style='padding:9px 12px;text-align:left;border-bottom:2px solid #ddd'>Status</th>"
                    f"<th style='padding:9px 12px;text-align:left;border-bottom:2px solid #ddd'>Em falta</th>"
                    f"<th style='padding:9px 12px;text-align:left;border-bottom:2px solid #ddd'>Foto</th>"
                    f"</tr></thead><tbody>{_prods_rows}</tbody></table></div>"
                )

            _now_str = datetime.now().strftime('%d/%m/%Y %H:%M')
            return (
                f"<!DOCTYPE html><html><head><meta charset='utf-8'>"
                f"<style>body{{font-family:Arial;background:#fff;padding:24px;color:#222;max-width:1000px;margin:auto}}"
                f".hdr{{display:flex;align-items:center;justify-content:space-between;"
                f"border-bottom:4px solid #ff2b2b;padding-bottom:16px;margin-bottom:24px}}"
                f".brand{{color:#ff2b2b;font-size:24px;font-weight:900;letter-spacing:1px}}"
                f".sub{{color:#888;font-size:13px;margin-top:4px}}"
                f".ftr{{color:#bbb;font-size:11px;margin-top:32px;text-align:center;"
                f"border-top:1px solid #eee;padding-top:14px}}"
                f"</style></head><body>"
                f"<div class='hdr'><div><div class='brand'>EL KAM MERCHANDISING</div>"
                f"<div class='sub'>{subtitulo}</div></div>"
                f"<div style='color:#ff2b2b;font-size:36px'>🏪</div></div>"
                f"<h2 style='color:#222;font-size:17px;margin-bottom:20px'>{titulo}</h2>"
                f"{_blocos}"
                f"<div class='ftr'>© El Kam Merchandising — Gerado em {_now_str} — Todos os direitos reservados.</div>"
                f"</body></html>"
            )
        ...
        if df_r.empty:
            ...
            for _merc_r in sorted(_mercs_rel):
                ...
                _data_r  = str(_df_merc_r.iloc[0]["data"])[:10] if not _df_merc_r.empty else ""
        ...
        for merc in dados_dia["mercado"].unique():
            ...
            if _ja_enviado:
                ...
                if st.button("✅ Enviar relatório", key=f"btn_{merc}_{dia_sel}",
                    ...
                    _prods_wa_list = []  # para montar mensagem WhatsApp
                    for _, _rp2 in prods.iterrows():
                        ...
                        if os.path.exists(_fp2):
                            with open(_fp2,"rb") as _fb2:
                                _fb64_prod = _b64.b64encode(_fb2.read()).decode()
                        # Status do produto: em_falta se nesse produto específico, senão status geral
                        _st_prod = "falta" if (_prod_nm in prod_faltante) else status_val
                        _pf_prod = _prod_nm if _st_prod == "falta" else ""
                        db_exec(
                            """INSERT INTO relatorio
                               (data,hora,funcionario,mercado,produto,status,foto,produto_faltante,foto_b64,enviado_mercado,notif_admin)
                               VALUES(?,?,?,?,?,?,?,?,?,0,0)""",
