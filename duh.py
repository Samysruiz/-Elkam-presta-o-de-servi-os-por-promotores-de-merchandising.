...
if tipo_efetivo == "admin":
    ...
    # ── RELATÓRIOS ────────────────────────────────────────
        ...
        def _gerar_html_rel(df_rows, titulo="El Kam Merchandising", subtitulo="", apagar_fotos=False):
            """Gera HTML profissional com fotos embutidas."""
            rows_html = ""
            for _, row in df_rows.iterrows():
                _st_raw = str(row.get("status","")).strip()
                _badge  = badge_map.get(_st_raw, _st_raw if _st_raw else "—")
                _pf     = row.get("produto_faltante","") or ""
                _hora   = row.get("hora","") or ""
                _b64v   = row.get("foto_b64","") or ""
                _fotos_tags = ""
                if _b64v:
                    for _fb64 in _b64v.split("||"):
                        if _fb64.strip():
                            _fotos_tags += f'<img src="data:image/jpeg;base64,{_fb64.strip()}" style="width:110px;border-radius:6px;margin:2px">'
                if not _fotos_tags:
                    _fotos_tags = "—"
                if apagar_fotos:
                    db_exec("UPDATE relatorio SET foto_b64='',foto='' WHERE rowid=?", (int(row.get("rowid",0)),))
                rows_html += (f"<tr><td>{str(row['data'])[:10]}</td>"
                              f"<td>{_hora}</td>"
                              f"<td>{row['funcionario']}</td>"
                              f"<td>{row['mercado']}</td>"
                              f"<td>{_badge}</td>"
                              f"<td>{_pf or '—'}</td>"
                              f"<td>{_fotos_tags}</td></tr>")
            return f"""<!DOCTYPE html><html><head><meta charset='utf-8'><style>
                body{{font-family:Arial;background:#fff;padding:24px;color:#222;max-width:1100px;margin:auto}}
                .header{{display:flex;align-items:center;justify-content:space-between;
                         border-bottom:3px solid #ff2b2b;padding-bottom:16px;margin-bottom:20px}}
                .brand{{color:#ff2b2b;font-size:22px;font-weight:900;letter-spacing:1px}}
                .sub{{color:#888;font-size:13px;margin-top:4px}}
                h2{{color:#222;font-size:16px;margin:20px 0 10px}}
                table{{width:100%;border-collapse:collapse;font-size:13px}}
                th{{background:#ff2b2b;color:#fff;padding:9px 12px;text-align:left;font-size:12px;letter-spacing:.5px}}
                td{{border-bottom:1px solid #eee;padding:8px 12px;vertical-align:middle}}
                tr:nth-child(even){{background:#f9f9f9}}
                .footer{{color:#bbb;font-size:11px;margin-top:24px;text-align:center;
                          border-top:1px solid #eee;padding-top:12px}}
                </style></head><body>
                <div class="header">
                    <div><div class="brand">EL KAM MERCHANDISING</div>
                    <div class="sub">{subtitulo}</div></div>
                    <div style="color:#ff2b2b;font-size:28px">🏪</div>
                </div>
                <h2>{titulo}</h2>
                <table><tr><th>Data</th><th>Hora</th><th>Funcionário</th><th>Mercado</th>
                <th>Status</th><th>Produto em falta</th><th>Fotos</th></tr>
                {rows_html}</table>
                <div class="footer">© El Kam Merchandising — Relatório gerado em {datetime.now().strftime('%d/%m/%Y %H:%M')} — Todos os direitos reservados.</div>
                </body></html>"""
        ...
        if df_r.empty:
            ...
            selecionados_rel = []
            for _, row in df_r.sort_values(["data_dt","hora"] if "hora" in df_r.columns else ["data_dt"], ascending=False).iterrows():
                _badge  = badge_map.get(str(row.get("status","")).strip(), "—")
                _pf     = row.get("produto_faltante","") or ""
                _hora   = row.get("hora","") or ""
                _enviado = int(row.get("enviado_mercado", 0) or 0)
                _env_tag = "<span style='color:#22c55e;font-size:11px;font-weight:700'>✅ Enviado</span>" if _enviado else "<span style='color:#555;font-size:11px'>⏳ Não enviado</span>"
                _has_foto = bool((row.get("foto_b64","") or "").strip())
                _foto_tag = "📷" if _has_foto else "—"

                _ck, _ci = st.columns([1, 11])
                with _ck:
                    if st.checkbox("", key=f"rel_{row['rowid']}", label_visibility="collapsed"):
                        selecionados_rel.append(int(row["rowid"]))
                with _ci:
                    st.markdown(
                        f"<div style='background:#111;border:1px solid {'#22c55e33' if _enviado else '#1c1c1c'};border-radius:8px;padding:10px 14px;margin-bottom:4px'>"
                        f"<div style='display:flex;justify-content:space-between;align-items:center'>"
                        f"<span style='color:#fff;font-weight:700'>{row['funcionario']}</span>"
                        f" <span style='color:#444'>·</span>"
                        f" <span style='color:#aaa'>{row['mercado']}</span>"
                        f" <span style='color:#444'>·</span>"
                        f" <span style='color:#555;font-size:12px'>{str(row['data'])[:10]} {_hora}</span>"
                        f"<span style='font-size:12px'>{_foto_tag}  {_badge}  {_env_tag}</span></div>"
                        + (f"<div style='color:#f59e0b;font-size:12px;margin-top:4px'>⚠️ Em falta: {_pf}</div>" if _pf else "")
                        + "</div>",
                        unsafe_allow_html=True)
    ...
    # ── AGENDA ──
        ...
        for merc in dados_dia["mercado"].unique():
            ...
            if st.button("✅ Enviar relatório", key=f"btn_{merc}_{dia_sel}",
                ...
                # Embute TODAS as fotos em base64 no banco (não só a primeira)
                import base64 as _b64
                _fotos_b64_list = []
                for _, _rp2 in prods.iterrows():
                    _fp2 = f"fotos/{usuario}/{merc}_{dia_sel}_{_rp2['produto']}_{date.today()}.jpg".replace(" ","_")
                    if os.path.exists(_fp2):
                        with open(_fp2,"rb") as _fb2: _fotos_b64_list.append(_b64.b64encode(_fb2.read()).decode())
                _foto_b64 = "||".join(_fotos_b64_list)  # múltiplas fotos separadas por ||
                _hora_agora = datetime.now().strftime("%H:%M")
                db_exec("""INSERT INTO relatorio
                    (data,hora,funcionario,mercado,produto,status,foto,produto_faltante,foto_b64,enviado_mercado)
                    VALUES(?,?,?,?,?,?,?,?,?,0)""",
                    (str(date.today()), _hora_agora, usuario, merc, "varios",
                     status_val, caminho, prod_faltante, _foto_b64))
                ...
                if _admin_tel:
                    ...
                    _msg_wa = (f"📋 *Relatório El Kam*\n"
                               f"👤 {usuario.split('.')[0].capitalize()}\n"
                               f"🏪 {merc}\n"
                               f"📅 {date.today().strftime('%d/%m/%Y')} {_hora_agora}\n"
                    ...
                    st.markdown(
                        ...
                        <div style='color:#22c55e;font-size:12px;font-weight:700;margin-bottom:8px'>
                            📲 Enviar relatório ao admin pelo WhatsApp</div>
                        <div style='display:flex;gap:8px'>
                        <a href='{_wa_m}' target='_blank' rel='noopener noreferrer' style='flex:1'>
                            <button style='background:#25D366;color:#fff;border:none;border-radius:8px;
                            padding:10px;font-size:12px;font-weight:700;cursor:pointer;width:100%'>
                            📲 Celular</button></a>
                        <a href='{_wa_p}' target='_blank' rel='noopener noreferrer' style='flex:1'>
                            <button style='background:#128C7E;color:#fff;border:none;border-radius:8px;
                            padding:10px;font-size:12px;font-weight:700;cursor:pointer;width:100%'>
                            💻 PC</button></a>
