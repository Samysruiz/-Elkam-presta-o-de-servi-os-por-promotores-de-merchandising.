...
if tipo_efetivo == "admin":
    ...
    if menu == "Dashboard":
        ...
        _ra = db_read("SELECT * FROM relatorio WHERE funcionario NOT IN (SELECT usuario FROM usuarios WHERE tipo='teste')"); _ca = db_read("SELECT * FROM checkin WHERE funcionario NOT IN (SELECT usuario FROM usuarios WHERE tipo='teste')")
    ...
    # ── FUNCIONÁRIOS ──────────────────────────────────────
        ...
        # ── Botão rápido para criar funcionário de teste ──
        st.markdown("<div style='margin-top:8px'></div>", unsafe_allow_html=True)
        if "exp_cad_teste" not in st.session_state: st.session_state["exp_cad_teste"] = False
        if st.button(
            ("🔼 Criar funcionário de teste" if st.session_state["exp_cad_teste"]
             else "🧪 Criar funcionário de teste"),
            key="btn_exp_cad_teste"
        ):
            st.session_state["exp_cad_teste"] = not st.session_state["exp_cad_teste"]
            st.rerun()
        if st.session_state["exp_cad_teste"]:
            _tn1, _tn2 = st.columns(2)
            with _tn1: _t_nome = st.text_input("Nome do teste", placeholder="ex: teste", key="t_nome")
            with _tn2: _t_sobr = st.text_input("Sobrenome", placeholder="ex: demo",    key="t_sobr")
            st.caption("🧪 Este usuário será marcado como **TESTE** — "
                       "não aparece nas métricas e ao excluir tudo é apagado (relatórios inclusos).")
            if st.button("Criar usuário de teste", key="btn_criar_teste"):
                if not _t_nome.strip() or not _t_sobr.strip():
                    st.error("Preencha nome e sobrenome.")
                else:
                    _t_login = f"{_t_nome.strip().lower()}.{_t_sobr.strip().lower()}"
                    if not db_read("SELECT usuario FROM usuarios WHERE usuario=?", (_t_login,)).empty:
                        st.error(f"Login '{_t_login}' já existe.")
                    else:
                        db_exec(
                            "INSERT INTO usuarios (usuario,senha,tipo,primeiro_acesso,telefone,aniversario) "
                            "VALUES(?,?,?,?,?,?)",
                            (_t_login, "elkam", "teste", 1, "", ""))
                        st.success(f"🧪 Teste criado! Login: **{_t_login}** · Senha: **elkam**")
                        st.session_state["exp_cad_teste"] = False
                        st.rerun()
        ...
        if funcs.empty:
            ...
            for _, row in funcs.iterrows():
                ...
                _is_teste = (str(row.get("tipo","")) == "teste")
                _teste_tag = (" <span style='background:#1a1a00;border:1px solid #f59e0b55;"
                ...
                if st.session_state.get(f"confirm_del_func_{login_func}", False):
                    if _is_teste:
                        st.warning(f"🧪 Excluir teste **{login_func}**? **TUDO será apagado** — usuário, agenda, checkins, relatórios e chat.")
                    else:
                    ...
                    with _cd1:
                        if st.button("✅ Confirmar exclusão", key=f"ok_del_{login_func}", use_container_width=True):
                            fazer_backup()
                            if _is_teste:
                                # Funcionário de teste: apaga TUDO sem deixar rastro
                                db_exec("DELETE FROM usuarios WHERE usuario=?", (login_func,))
                                ...
                                db_exec("DELETE FROM relatorio WHERE funcionario=?", (login_func,))
                                db_exec("DELETE FROM auditoria WHERE funcionario=?", (login_func,))
                                st.session_state.pop(f"confirm_del_func_{login_func}", None)
                                st.success(f"🧪 Usuário de teste **{login_func}** excluído — todos os dados apagados.")
                            else:
    ...
    # ── CHAT ADMIN ────────────────────────────────────────
        ...
        funcs_ch = db_read("SELECT usuario FROM usuarios WHERE tipo IN ('funcionario','teste') AND usuario!='superadmin'")
    ...
    # ── AUDITORIA ──────────────────────────────────────────
        ...
        _rel_all_a = db_read("SELECT * FROM relatorio WHERE funcionario NOT IN (SELECT usuario FROM usuarios WHERE tipo='teste')")
        _ci_all_a  = db_read("SELECT * FROM checkin WHERE funcionario NOT IN (SELECT usuario FROM usuarios WHERE tipo='teste')")
