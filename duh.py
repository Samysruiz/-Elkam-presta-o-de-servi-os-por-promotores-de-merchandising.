    ...
    with col_form:
        ...
        if st.session_state["tela_login"] == "login":
            ...
            if st.button("ENTRAR →", use_container_width=True):
                ...
                db_exec("SELECT * FROM usuarios WHERE LOWER(usuario)=LOWER(?) AND senha=?",
                          (u, senha_input))
                user = c.fetchone()
                if user:
...
if tipo == "admin":
    ...
    if menu == "Dashboard":
        ...
        rel = db_read("SELECT * FROM relatorio")
        hoje         = date.today()
        ...
        nao_lidas = db_read(
            "SELECT COUNT(*) as t FROM chat WHERE tipo='funcionario' AND lido=0").iloc[0]["t"]
        if nao_lidas > 0:
    ...
    # ── FUNCIONÁRIOS ──────────────────────────────────────
        ...
        funcs = db_read("SELECT usuario, telefone FROM usuarios WHERE tipo='funcionario'")
            ...
            for _, row in funcs.iterrows():
                ...
                with col_e:
                    if st.button("🗑️", key=f"del_{login_func}"):
                        fazer_backup()
                        db_exec("DELETE FROM usuarios WHERE usuario=? AND tipo='funcionario'", (login_func,))
    ...
    # ── AGENDA ────────────────────────────────────────────
        ...
        funcs_ag = db_read("SELECT usuario FROM usuarios WHERE tipo='funcionario'")
    ...
    # ── CHAT ADMIN ────────────────────────────────────────
        ...
        funcs_ch = db_read("SELECT usuario FROM usuarios WHERE tipo='funcionario'")
    ...
    # ── CONFIGURAÇÕES ─────────────────────────────────────
        ...
        admin_row = db_read(
            "SELECT email FROM usuarios WHERE tipo='admin' AND usuario='admin'")
