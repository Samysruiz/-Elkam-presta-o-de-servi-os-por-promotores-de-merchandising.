...
if tipo_efetivo == "admin":
    ...
    # ── DESTINATÁRIOS PDF ─────────────────────────────────
        ...
        with st.expander("➕ Novo cliente", expanded=False):
            ...
            with _c4: _cl_pw    = st.text_input("Senha inicial", key="cl_pw", placeholder="senha123")
            ...
            if st.button("💾 Cadastrar cliente", key="cl_add"):
                if not _cl_nome.strip() or not _cl_user.strip() or not _cl_pw.strip():
                    ...
                    db_exec("INSERT INTO clientes (nome,email,usuario,senha,mercados,ativo) VALUES(?,?,?,?,?,1)",
                            (_cl_nome.strip(), _cl_email.strip(), _cl_user.strip(),
                             _cl_pw.strip(), ",".join(_cl_mercs_sel)))
                    st.success(f"✅ Cliente **{_cl_nome.strip()}** cadastrado! Login: `{_cl_user.strip()}`")
                    st.rerun()
        ...
        if _cls.empty:
            ...
            for _, _cl in _cls.iterrows():
                _c_a, _c_b, _c_c = st.columns([5, 2, 1])
                with _c_a:
                    _m_txt = _cl.get("mercados","") or "Todos os mercados"
                    _s_txt = "🟢 Ativo" if _cl.get("ativo",1) else "🔴 Inativo"
                    st.markdown(
                        f"<div style='background:#111;border:1px solid #222;border-radius:8px;"
                        f"padding:10px 14px;margin:3px 0'>"
                        f"<span style='color:#fff;font-weight:700'>{_cl['nome']}</span>"
                        f"<span style='color:#555;font-size:12px'> · {_s_txt}</span><br>"
                        f"<span style='color:#444;font-size:11px'>🔑 {_cl['usuario']} · 📧 {_cl.get('email','') or '—'}</span><br>"
                        f"<span style='color:#333;font-size:11px'>🏪 {_m_txt}</span></div>",
                        unsafe_allow_html=True)
                with _c_b:
                    st.code(_cl['usuario'] + "\n" + _cl['senha'], language=None)
                with _c_c:
                    if st.button("🗑️", key=f"cl_del_{_cl['id']}", use_container_width=True):
                        db_exec("DELETE FROM clientes WHERE id=?", (int(_cl["id"]),))
                        st.rerun()
            ...
            if st.button("✅ Enviar relatório", key=f"btn_{merc}_{dia_sel}",
                ...
                for _, _rp2 in prods.iterrows():
                    ...
                    db_exec(
                        ...
                        "produto_faltante,foto_b64,hora,enviado_mercado,notif_admin,lat,lon,foto_hash) "
                        "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        (_data_hoje, usuario, merc, _prod_nm, _st_prod, _fp2,
                         _pf_prod, _fb64_prod, _hora_agora, 0, 1,
                         _gps_lat_v, _gps_lon_v, _foto_hash))
