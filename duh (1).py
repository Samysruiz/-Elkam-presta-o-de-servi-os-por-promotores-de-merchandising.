...
if tipo_efetivo == "admin":
    ...
    with st.sidebar:
        # ── Sair no TOPO — sempre visível sem precisar rolar ──
        if st.button("🚪 Sair", key="adm_sair_btn", use_container_width=True):
            for _k in list(st.session_state.keys()):
                del st.session_state[_k]
            st.rerun()
        st.markdown("<div style='border-top:1px solid #1c1c1c;margin:8px 0 10px'></div>",
                    unsafe_allow_html=True)
